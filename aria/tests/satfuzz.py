...
while len(g_unfinished1) > 0:
    ...
    if n0.refs >= g_min_refs - 1:
        if g_unfinished1.__contains__(str(id0)):
            del g_unfinished1[str(id0)]
    if g_unfinished2.__contains__(str(id0)):
        del g_unfinished2[str(id0)]

    if n1.refs >= g_min_refs - 1:
        if g_unfinished1.__contains__(str(id1)):
            del g_unfinished1[str(id1)]
    if g_unfinished2.__contains__(str(id1)):
