...
def _init(comments):
    ...
    if _options.ratio is not None:
        ...
        if _options.ratio < 0.0 or _options.ratio > 1.0:
            ...
            parser.error("ratio will produce empty formula. retry with --no-reduce")
    ...
    if _options.num_clauses is None:
        if _options.ratio is not None:
            lo_bound = max(ex_vars, un_vars)
        else:
            lo_bound = 1

        _options.num_clauses = random.randint(lo_bound, max(lo_bound, MAX_CLAUSES))
    else:
...
def _get_num_variables(quantifier):
    ...
    return len(_qcache[quantifier][USED]) + len(_qcache[quantifier][UNUSED])
...
def _clean_up_formula(num_vars, quantsets, clauses):
    ...
    var_offs = [0 for _ in range(0, num_vars + 1)]
...
def qbfuzz_main():
    ...
    if _options.reduce or not _options.allow_unit or _options.free_ratio is not None:
        # save new number of variables (unused variables may be removed)
        _options.num_vars = _clean_up_formula(_options.num_vars, quantsets, clauses)
