...
class TestBVSat(TestCase):
    ...
    def test_bvsat_simple_constraints(self):
        ...
        fml = z3.And(x > 10, y > 20, z < 15, x + y + z == 50, x * 2 == y - 5)
        sol = QFBVSolver()
    ...
    def test_bvsat_contradiction(self):
        ...
        fml = z3.And(x == y, x != y, x + y == 100, x * 2 == y * 2)
        sol = QFBVSolver()
        result = sol.solve_smt_formula(fml)
    ...
    def test_bvsat_shift_operations(self):
        ...
        x, y = z3.BitVecs("x y", 8)
        fml = z3.And(
            (x << 2) == 20, (y >> 1) == x, x > 0, y > 0  # Left shift  # Right shift
        )
        sol = QFBVSolver()
        result = sol.solve_smt_formula(fml)
        self.assertEqual(result, SolverResult.SAT)
    ...
    def test_bvsat_rotation(self):
        ...
        x, y = z3.BitVecs("x y", 8)
        fml = z3.And(z3.RotateLeft(x, 2) == 20, z3.RotateRight(y, 3) == x, x > 0, y > 0)
        sol = QFBVSolver()
        result = sol.solve_smt_formula(fml)
        self.assertEqual(result, SolverResult.SAT)
    ...
    def test_bvsat_multiple_variables(self):
        ...
        fml = z3.And(x + y + z == 100, x > 10, y > 20, z > 30)
        sol = QFBVSolver()
        result = sol.solve_smt_formula(fml)
        self.assertEqual(result, SolverResult.SAT)
