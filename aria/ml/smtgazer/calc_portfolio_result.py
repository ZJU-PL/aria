...
def run_seed(seed_val, category_key):
    ...
    if dataset_name == "SyGuS":
        c_num = 3
    test_file = (f"output/test_result_{category_key}_{seed_val}_"
                 f"{c_num}.json")
    ...
    if dataset_name in ('QF_Bitvec', 'Equality+LinearArith',
                        'QF_NonLinearIntArith'):
        solver_list = list(solver_dict[dataset_name])
    elif dataset_name in ('BMC', 'SymEx'):
        solver_list = ['Bitwuzla', 'STP 2021.0',
                       'Yices 2.6.2 for SMTCOMP 2021', 'cvc5',
                       'mathsat-5.6.6', 'z3-4.8.11']
    elif dataset_name == "SyGuS":
        solver_list = ['cvc5', 'UltimateEliminator+MathSAT-5.6.6',
                       'smtinterpol-2.5-823-g881e8631', 'veriT',
                       'z3-4.8.11']
    else:
        solver_list = []
...
if __name__ == '__main__':
    key_set = ['Equality+LinearArith', 'QF_NonLinearIntArith', 'QF_Bitvec',
               'SyGuS', 'BMC', 'SymEx']
