...
class GA:
    ...
    def __init__(self, population_size=None, retain_percentage=None):
        ...
        self._population = [
            TacticSeq.random().mutate() for _ in range(self._population_size)
        ]
        self._new = []
        self._retained = []
        self._generation = 0
    ...
    def repopulate(self):
        ...
        while len(self._population) < self._population_size:
            ...
            if (
                i1 != i2
                and self._new[i1].fitness != penalty
                and self._new[i2].fitness != penalty
            ):
    ...
    def get_best_sequence(self):
        ...
        return (
            min(all_individuals, key=lambda x: x.fitness) if all_individuals else None
        )
    ...
    def run_evolution(
        ...
        for gen in range(generations):
            ...
            gen_stats = self.run_generation(
                mode=mode,
                smtlib_file=smtlib_file,
                timeout=timeout,
