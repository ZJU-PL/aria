...
def run_genetic_algorithm(args):
    ...
    try:
        ...
        print(
            f"Generations: {results['generations_run']}, "
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=[EvaluationMode.PYTHON_API, EvaluationMode.BINARY_Z3],
        default=EvaluationMode.PYTHON_API,
        help="Evaluation mode (default: python_api)",
    )
    parser.add_argument(
        "--population", "-p", type=int, default=64, help="Population size (default: 64)"
    )
    parser.add_argument(
        "--generations", "-g", type=int, default=128, help="Generations (default: 128)"
    )
    parser.add_argument(
        "--timeout", "-t", type=int, default=8, help="Timeout in seconds (default: 8)"
    )
    parser.add_argument("--smtlib-file", help="SMT-LIB2 file for evaluation")
    parser.add_argument(
        "--output-dir", "-o", default=".", help="Output directory (default: .)"
    )
    parser.add_argument(
        "--save-interval",
        type=int,
    ...
    parser.add_argument(
        "--demo", action="store_true", help="Show demo instead of running GA"
    )
