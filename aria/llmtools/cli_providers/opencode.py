...
from aria.llmtools.cli_providers.base import LLMResponse, ToolCallRequest
    ...
    for attempt in range(max_retries):
        try:
            ...
            if is_500_error and attempt < max_retries - 1:
                ...
                logger.warning(
                    f"OpenCode 500 error (attempt {attempt + 1}/{max_retries}), retrying in {delay}s: {error_str}"
                )
                ...
                retry_info = (
                    f" (failed after {attempt + 1} attempts)" if attempt > 0 else ""
                )
