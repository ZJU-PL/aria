...
class CNFGenerator:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, grammar_rules):
        ...
        for rule_index in range(len(self.grammar_rules)):
            if self.grammar_rules[rule_index][0] not in self.grammar_nonterminals_map:
                self.grammar_nonterminals_map[self.grammar_rules[rule_index][0]] = {
                    rule_index
                }
            else:
                self.grammar_nonterminals_map[self.grammar_rules[rule_index][0]].add(
                    rule_index
                )
        ...
        while f:
            ...
            self.unitary = {
                ...
                for rule in self.grammar_rules
                if (
                    isinstance(rule[1], six.string_types)
                    and rule[1] in self.grammar_nonterminals
                )
                or (
                    not isinstance(rule[1], six.string_types)
                    and len(rule[1]) == 1
                    and rule[1][0] in self.grammar_nonterminals
                )
            }
            ...
            for u in self.unitary:
                ...
                if ui in self.grammar_nonterminals_map:
                    for rule_index in self.grammar_nonterminals_map[ui]:
                        if (
                            u[0],
                            self.grammar_rules[rule_index][1],
                        ) not in self.grammar_rules:
                            f = 1
                            self.grammar_rules.append(
                                (u[0], self.grammar_rules[rule_index][1])
                            )
                            self.grammar_nonterminals_map[u[0]].add(
                                len(self.grammar_rules) - 1
                            )
        for u in self.unitary:
        ...
        for a in self.grammar_terminals:
            for rule_index in range(len(self.grammar_rules)):
                if (
                    not isinstance(self.grammar_rules[rule_index][1], six.string_types)
                    and len(self.grammar_rules[rule_index][1]) >= 2
                    and a in self.grammar_rules[rule_index][1]
                ):
                    ...
                    self.grammar_rules[rule_index] = (
                        self.grammar_rules[rule_index][0],
                        tuple(symbolslist),
                    )
    ...
    def next_rule(self, left_hand_side, right_hand_side, rule_index):
        if (
            not isinstance(right_hand_side, six.string_types)
            and len(right_hand_side) == 2
        ):
...
class CFGGenerator(object):
    ...
    def _check_intemediate(self, myntr, maxstate):
        ...
        while r < maxstate:
            if r != i and r != j:
                if (
                    "A" + i + "," + repr(r) not in self.resolved
                    and "A" + j + "," + repr(r) in self.resolved
                ):
                    self.resolved["A" + i + "," + repr(r)] = (
                        self.resolved[myntr] + self.resolved["A" + j + "," + repr(r)]
                    )
                    ...
                    find = 1
                if (
                    "A" + repr(r) + "," + j not in self.resolved
                    and "A" + repr(r) + "," + i in self.resolved
                ):
                    self.resolved["A" + repr(r) + "," + j] = (
                        self.resolved["A" + repr(r) + "," + i] + self.resolved[myntr]
                    )
    ...
    def _check_self_replicate(self, myntr):
        ...
        for nonterm in self.grammar.grammar_nonterminals_map:
            for i in self.grammar.grammar_nonterminals_map[nonterm]:
                if (
                    self.grammar.grammar_rules[i][0] not in self.resolved
                    and not isinstance(self.grammar.grammar_rules[i][1], (set, tuple))
                    and self.grammar.grammar_rules[i][1] == myntr
                ):
                    self.resolved[self.grammar.grammar_rules[i][0]] = self.resolved[
                        myntr
                    ]
    ...
    def _gen(self, optimized, splitstring):
        ...
        for nt in self.grammar.grammar_nonterminals_map:
            for i in self.grammar.grammar_nonterminals_map[nt]:
                if self.grammar.grammar_rules[i][
                    0
                ] not in self.resolved and not isinstance(
                    self.grammar.grammar_rules[i][1], (set, tuple)
                ):
                    if (
                        self.grammar.grammar_rules[i][1] != "@empty_set"
                        and self.grammar.grammar_rules[i][1]
                        in self.grammar.grammar_terminals
                    ):
            ...
            if not change:
                while not change and len(self.bfs_queue) > 0:
                    ...
                    if ret == 1:
                        ...
                        return ret
                    if optimized and self._check_intemediate(myntr, self.maxstate):
                        change = 1
                        break
