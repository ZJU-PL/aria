...
class Regex:
        ...
        for state_a in self.mma.states:
            for target in self.A[state_a.stateid]:
                if len(self.A[state_a.stateid][target]) == 0:
                    ...
                    len([a for a in self.A[state_a.stateid][target] if len(a) > 1]) == 0
                    and len(self.A[state_a.stateid][target]) > len(self.alphabet) / 2
                ):
                    self.A[state_a.stateid][target] = (
                        "[^"
                        + "".join(
                    ...
                    self.A[state_a.stateid][target] = (
                        "[" + "".join(self.A[state_a.stateid][target]) + "]"
                    )
                elif (
                    len([a for a in self.A[state_a.stateid][target] if len(a) > 1]) > 0
                ):
    ...
    def checkIfSameStateNotNeeded(self, state, same):
        ...
        try:
            if (
                len(self.A[state]["same"]) > 3
                and self.A[state]["same"][0] == "("
                and self.A[state]["same"][
                    len(self.A[state]["same"]) - 1 : len(self.A[state]["same"]) + 1
                ]
            ):
        ...
        for i in self.A:
            ...
            for target in self.A[i]:
                if target < i:
                    ...
                    if exists and (
                        transfer_value == self.A[i][target]
                        or (
                            target in self.A[target]
                            and (
                                self.A[target][target] == self.A[i][target]
                                or len(
                                    set(self.A[i][target]) - set(self.A[target][target])
                                )
                                == 0
                            )
                        )
        ...
        for state_a in self.A.keys():
            ...
            if state_a in self.A[state_a]:
                ...
                if "same" in self.A[state_a]:
                    ...
                    self.A[state_a]["same"] = (
                        "("
                        + "".join(self.replaceAlphabet(self.A[state_a][state_a]))
                        + ")*"
                        + same
                    )
                ...
                for state_b in self.A:
                    if state_b != state_a and state_b in self.A:
                        if state_a in self.A[state_b]:
                            if (
                                "rest" in self.A[state_b]
                                and self.A[state_b]["rest"] != ""
                                and self.A[state_b]["rest"]
                                == self.cleaner(self.A[state_a]["string"])
                            ):

                                if "string" not in self.A[state_b]:
                                    self.A[state_b]["string"] = (
                                        "("
                                        + self.A[state_b][state_a]
                                        + ")?"
                                        + self.A[state_b]["rest"]
                                    )
                                else:
            ...
            if (
                state_a != start
                and state_a in self.A
                and len(self.A[state_a].keys()) == 1
            ...
            if (
                state_a in self.A
                and len(self.A[state_a].keys()) == 2
                and "string" in self.A[state_a]
                and "rest" in self.A[state_a]
            ):
                if self.A[state_a]["rest"] != "":
                    self.A[state_a]["string"] = (
                        self.cleaner(self.A[state_a]["string"])
                        + "("
                        + self.replaceAlphabet(self.A[state_a]["rest"])
                        + ")?"
                    )
                del self.A[state_a]["rest"]
                found = 1
            ...
            if (
                ...
                ) + self.cleaner(self.A[state_a]["rest"])
            ...
            if (
                ...
                self.A[state_a]["string"] = (
                    self.checkIfSameStateNotNeeded(
                        state_a, self.cleanerS(self.A[state_a]["same"])
                    )
                    + self.cleaner(self.A[state_a]["string"])
                    + add
    ...
    def fixBrzozowskiAdvanced(self, found):
        ...
        for state_a in self.A:
            ...
            if (
                ...
                for state_b in [
                    x.stateid
                    for x in self._bfs_sort(
                        sorted(
                            self.mma.states, key=attrgetter("initial"), reverse=True
                        )[0]
                    )
                ]:
                    if state_b != state_a and state_b in self.A:
