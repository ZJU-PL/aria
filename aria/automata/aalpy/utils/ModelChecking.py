...
import aria.automata.aalpy.paths
from aria.automata.aalpy.SULs import AutomatonSUL
from aria.automata.aalpy.automata import Mdp, StochasticMealyMachine, MealyMachine, Dfa, MooreMachine, MooreState, MealyState, \
    DfaState
from aria.automata.aalpy.base import DeterministicAutomaton, SUL, AutomatonState
...
def get_properties_file(exp_name):
    property_files = {
        'first_grid': aria.automata.aalpy.paths.path_to_properties + 'first_eval.props',
        'second_grid': aria.automata.aalpy.paths.path_to_properties + 'second_eval.props',
        'shared_coin': aria.automata.aalpy.paths.path_to_properties + 'shared_coin_eval.props',
        'slot_machine': aria.automata.aalpy.paths.path_to_properties + 'slot_machine_eval.props',
        'mqtt': aria.automata.aalpy.paths.path_to_properties + 'emqtt_two_client.props',
        'tcp': aria.automata.aalpy.paths.path_to_properties + 'tcp_eval.props',
        'bluetooth': aria.automata.aalpy.paths.path_to_properties + 'bluetooth.props',
    }
...
def evaluate_all_properties(prism_file_name, properties_file_name):
    ...
    prism_file = aria.automata.aalpy.paths.path_to_prism.split('/')[-1]
    path_to_prism_file = aria.automata.aalpy.paths.path_to_prism[:-len(prism_file)]

    file_abs_path = path.abspath(prism_file_name)
    properties_als_path = path.abspath(properties_file_name)
    results = {}
    proc = subprocess.Popen(
        [aria.automata.aalpy.paths.path_to_prism, file_abs_path, properties_als_path],
...
def model_check_properties(model: Mdp, properties: str):
    ...
    from aria.automata.aalpy.utils import mdp_2_prism_format
...
def stop_based_on_confidence(hypothesis, property_based_stopping, print_level=2):
    ...
    """
    from aria.automata.aalpy.automata.StochasticMealyMachine import smm_to_mdp_conversion
...
def compare_automata(aut_1: DeterministicAutomaton, aut_2: DeterministicAutomaton, num_cex=10):
    ...
    """
    #
    from aria.automata.aalpy.oracles import RandomWMethodEqOracle
    # if you use this function with the same object (does not make sense anyway)
    if aut_1 is aut_2:
        aut_2 = aut_1.copy()
