...
def calc_coefficient_of_variation(mean, std):
    """
    The coefficient of variation is a statistical measure of the relative
...
def scipy_entropy_discrete(l, num_outcomes):
    """
    Create a probability distribution of l, and then get the entropy of
    that distribution
    :param l: Data
    :param num_outcomes: The total possible number of outcomes
    :return: Entropy of l
    """
    if sci_stats is None:
        raise ImportError("scipy.stats is required for this function")
    p = [0] * num_outcomes
    for elem in l:
        p[elem] += 1
...
def scipy_entropy_continous(l, buckets=100):
    ...
    that distribution
    :param l: Data
    :param buckets: Number of buckets for the distribution
    :return: Entropy of l
    """
    if sci_stats is None:
        raise ImportError("scipy.stats is required for this function")
...
def entropy_float_array(l, num, vals, maxval):
    ...
    """
    # seems to be off by 1 error in the sat implementation...
    # you shouldn't need 101 buckets...
    p = [0] * (vals + 1)
    entropy = 0
...
def entropy_int_array(l, number_of_outcomes):
    """
    Calculate entropy for integer array.
    :param l: List of statistics (vcg variable/clause node degrees, )
    :param number_of_outcomes: upper bound on the maximum number of outcomes
        (e.g. for vcg clause node degree, it could have a maximum of the
        number of variables (the clauses contains all variables)).
