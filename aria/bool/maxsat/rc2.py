...
class RC2(object):
    ...
    def init(self, formula, incr=False):
        ...
        self.oracle = Solver(
            name=self.solver, bootstrap_with=formula.hard, incr=incr, use_timer=True
        )
        ...
        if self.verbose > 1:
            print(
                "c formula: {0} vars, {1} hard, {2} soft".format(
    ...
    def delete(self):
        ...
        if self.oracle:
            if (
                not self.oracle.supports_atmost()
    ...
    def compute(self):
        ...
        if res:
            ...
            self.model = map(
                lambda l: int(copysign(self.vmap.i2e[abs(l)], l)), self.model
            )
            ...
            if model != None:
                if block == 1:
                    ...
                    self.oracle.add_clause(
                        [l for l in filter(lambda l: -l in m, self.sall)]
                    )
                else:
    ...
    def adapt_am1(self):
        ...
        if confl:  # filtering out unnecessary connections
            ...
            if self.verbose > 1:
                print(
                    "c unit cores found: {0}; cost: {1}".format(len(confl), self.cost)
                )
        ...
        if self.verbose > 1 and nof_am1:
            print(
                "c am1s found: {0}; avgsz: {1:.1f}; cost: {2}".format(
                    nof_am1, sum(len_am1) / float(nof_am1), self.cost
                )
            )
    ...
    def next_level(self):
        ...
        while self.levl < len(self.blop) - 1:
            ...
            if clu_str:
                ...
                sumc = sum(
                    [self.blop[l] * len(self.wstr[self.blop[l]]) for l in cluster]
                )
...
def usage():
    ...
    print(
        "        -b, --block=<string>     When enumerating MaxSAT models, how to block previous solutions"
    )
    print(
        "                                 Available values: mcs, model, mss (default = model)"
    )
    print("        -c, --comp=<string>      Enable one of the MSE18 configurations")
    print(
        "                                 Available values: a, b, none (default = none)"
    )
    ...
    print(
        "        -i, --incr               Use SAT solver incrementally (only for g3 and g4)"
    )
    print("        -l, --blo=<string>       Use BLO and stratification")
    print(
        "                                 Available values: basic, div, cluster, none, full (default = none)"
    )
    print(
        "        -m, --minimize           Use a heuristic unsatisfiable core minimizer"
    )
    print("        -s, --solver=<string>    SAT solver to use")
    print(
        "                                 Available values: g3, g4, lgl, mcb, mcm, mpl, m22, mc, mgh (default = g3)"
    )
