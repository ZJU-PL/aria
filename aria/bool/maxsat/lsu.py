...
class LSU:
    ...
    def _init(self, formula):
        ...
        self.oracle = Solver(
            name=self.solver, bootstrap_with=formula.hard, incr=True, use_timer=True
        )
    ...
    def _get_model_cost(self, formula, model):
        ...
        for i, cl in enumerate(formula.soft):
            cost += (
                ...
                if all(
                    l not in model_set
                    for l in filter(lambda l: abs(l) <= self.formula.nv, cl)
                )
                else 0
            )
...
class LSUPlus(LSU, object):
    ...
    def __init__(self, formula, solver, expect_interrupt=False, verbose=0):
        ...
        assert (
            solver in SolverNames.gluecard3
            or solver in SolverNames.gluecard4
            or solver in SolverNames.minicard
        ), "{0} does not support native cardinality constraints".format(solver)

        super(LSUPlus, self).__init__(
...
def parse_options():
    ...
    try:
        opts, args = getopt.getopt(
            sys.argv[1:], "hms:t:v", ["help", "model", "solver=", "timeout=", "verbose"]
        )
...
def print_usage():
    ...
    print(
        "                                 Available values: g3, g4, mc, m22, mgh (default = g4)"
    )
