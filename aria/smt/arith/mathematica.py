...
def has_kernel():
    """Check if Mathematica kernel is available."""
    try:
...
class MathematicaSession():
    ...
    @staticmethod
    def get_session():
        """Get or create Mathematica session."""
    ...
    def terminate_session():
        """Terminate the Mathematica session."""
...
class MathematicaSolver(Solver):
    ...
    def find_optimal(self, function, constraints, is_minimum=True, round_precision=6):
        ...
        for rule in res[1]:
            ...
            assert isinstance(rule.args[1], float)
...
class MathematicaConverter(Converter, DagWalker):
    ...
    def walk_symbol(self, formula, **kwargs):
        ...
        res = getattr(wolframclient.language.Global, sanitized)
    ...
    def walk_real_constant(self, formula, **kwargs):
        """Convert a real constant to Mathematica representation."""
    ...
    def walk_int_constant(self, formula, **kwargs):
        """Convert an integer constant (not supported)."""
        raise ConvertExpressionError(f"Integer constants ({formula}) are not allowed!")
...
class MathematicaQuantifierEliminator(QuantifierEliminator):
    """Quantifier eliminator using Mathematica."""
