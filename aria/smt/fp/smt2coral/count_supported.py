...
def main(args):
    ...
    parser.add_argument(
        "invocation_info_file",
        nargs="?",
        type=argparse.FileType("r"),
        default=sys.stdin,
    )
    parser.add_argument("--benchmark-base", type=str, dest="benchmark_base", default="")
    ...
    for index, benchmark_info in enumerate(results):
        ...
        if can_convert:
            _logger.info("%s: Conversion successful (%s)", benchmark, progress_str)
            benchmarks_can_be_converted.add(benchmark)
            if sound_translation:
                benchmarks_with_sound_translation.add(benchmark)
        else:
            _logger.warning("%s: Conversion failed (%s)", benchmark, progress_str)
            benchmarks_cannot_be_converted.add(benchmark)

    # Report
    print(f"# of benchmarks can be converted: " f"{len(benchmarks_can_be_converted)}")
    print(
        f"# of benchmarks that could be converted soundly: "
        f"{len(benchmarks_with_sound_translation)}"
    )
    print(
        f"# of benchmarks cannot be converted: "
