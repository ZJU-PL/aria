...
def _gen(expr_z3, symbolTable, cache, result):
    ...
    if _is_variable(expr_z3):
        ...
        if z3.is_int(expr_z3):
            ...
            symType = Sort.Float
            warnings.warn(
                "****WARNING****: Real variable '%s' treated as floating point" % symVar
            )
        else:
        ...
        if z3.is_fp(expr_z3) or z3.is_real(expr_z3):
            ...
            if isinstance(expr_z3, z3.FPNumRef):
                ...
                try:
                    ...
                    if (
                        expr_z3.isInf()
                        and expr_z3.decl().kind() == z3.Z3_OP_FPA_PLUS_INF
                    ):
                        str_ret = "INFINITY"
                    elif (
                        expr_z3.isInf()
                        and expr_z3.decl().kind() == z3.Z3_OP_FPA_MINUS_INF
                    ):
                        str_ret = "- INFINITY"
                    elif expr_z3.isNaN():
                        str_ret = "NAN"
    ...
    if z3.is_and(expr_z3):
        ...
        for i in range(1, expr_z3.num_args()):
            toAppendExpr = "BAND( %s,%s )" % (
                toAppendExpr,
                _gen(expr_z3.arg(i), symbolTable, cache, result),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "smt2_file",
        help="specify the smt2 file to analyze.",
    ...
    parser.add_argument(
        "--stepSize_round2", help="parameter of basinhopping", type=float, default=100.0
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--multi",
        help="multi-processing (default: false)",
        default=False,
        action="store_true",
    )
    parser.add_argument(
        "--multiMessage",
        help="multi-processing message",
        default=False,
        action="store_true",
    )
    parser.add_argument(
        "--round2",
        help="activate round2 when unsat (default: false)",
        default=False,
        action="store_true",
    )
    parser.add_argument(
    ...
    try:
        parsed = z3.parse_smt2_string(args.smt2_file.read())
        if len(parsed) == 1:
            expr_z3 = z3.simplify(parsed[0])
        else:
            expr_z3 = z3.simplify(z3.And(*parsed))
