...
def mcmc(args, i):
    ...
    if res.fun != 0 and res.fun < args.round2_threshold:
        ...
        res_round2 = op.basinhopping(
            obj_near,
            np.zeros(foo.dim),
            niter=200 * len(np.zeros(foo.dim)),
...
def nth_fp_vectorized(n, x):
    ...
    if m >= 0x7FF0000000000000:
        warnings.warn(
            "Value out of range, with n= %g,x=%g,m=%g, process=%g"
            % (n, x, m, mp.current_process.name)
        )
...
def scales():
    return [
        (lambda x: x**11, lambda x: np.sign(x) * np.abs(x) ** (1.0 / 11)),
...
def mcmc_round1(args):
    ...
    res = op.basinhopping(
        ...
        niter=200 * len(sp),
        stepsize=args.stepSize,
        minimizer_kwargs={"method": args.method},
        callback=_callback_global,
    )
    if args.showResult:
...
def mcmc_round2(args, X_star):
    ...
    for scale, scale_inv in scales():
        res = op.basinhopping(
            lambda x: foo_ulp.R(*scale(x)),
            scale_inv(X_star),
            niter=200 * len(scale_inv(X_star)),
            minimizer_kwargs={"method": args.method},
            callback=_callback_global,
            stepsize=args.round2_stepsize,
        )
...
def mcmc_round3(args, X_star):
    ...
    res = op.basinhopping(
        ...
        niter=200 * len(np.zeros(foo_ulp.dim)),
        minimizer_kwargs={"method": args.method},
        callback=_callback_global,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-v", "--version", action="version", version="%(prog) version 12/18/2015"
    )
    parser.add_argument(
        "--niter",
        help="niter in basinhopping",
        action="store",
        type=int,
        required=False,
        default=100,
    ...
    parser.add_argument(
        "--method",
        help="Local minimization procedure",
        default="powell",
        choices=[
            "powell",
            "slsqp",
    ...
    parser.add_argument(
        "--showTime",
        help="show the time-related info (default: false)",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--showResult",
        help="show the basinhopping output (default:false)",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--stepSize", help="parameter of basinhopping", type=float, default=10.0
    )
    parser.add_argument(
        "--round2_stepsize", help="parameter of basinhopping", type=float, default=100.0
    )
    parser.add_argument(
        "--verify", help="verify the model", action="store_true", default=False
    )
    parser.add_argument(
        "--verify2",
        help="verify the model (method 2)",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--showModel",
        help="show the model as a var->value mapping",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--showSymbolTable",
        help="show the symbol table, var->type",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--showConstraint",
        help="show the constraint, using the Z3 frontend",
        action="store_true",
        default=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="gcc -O3 -fbracket-depth=2048 -fPIC -I /usr/local/Cellar/python/3.13.7/Frameworks/Python.framework/Versions/3.13/include/python3.13/ %(file)s.c -dynamiclib -o %(file)s.so -L /usr/local/Cellar/python/3.13.7/Frameworks/Python.framework/Versions/Current/lib/ -lpython3.13",
    )
    parser.add_argument(
        "--startPoint",
        help="start point in a single dimension",
        action="store",
        type=float,
        default=1.0,
    )
    ...
    parser.add_argument(
        "--multi",
        help="multi-processing (default: true)",
        default=True,
        action="store",
        type=str2bool,
    )
    ...
    parser.add_argument(
        "--debug",
        help="debug mode (with verify and showresults, etc.)",
        default=True,
        action="store_true",
    )
    parser.add_argument(
        "--printModel", help="print the model", default=False, action="store_true"
    )
    parser.add_argument(
        "--bench", help="benchmarking mode", default=False, action="store_true"
    )
    parser.add_argument(
        "--genOnly",
        help="generate code only, without deciding satisfiability",
        default=False,
        action="store_true",
    )
    ...
    with open("XSAT_IN.txt") as f:
        try:
            ...
            sys.stderr.write(
                "[Xsat] The Z3 fornt-end fails when verifying the model.\n"
            )
    ...
    if not args.multi:
        ...
        satisfiable_round1 = verify_solution(
            expr_z3, X_star, symbolTable, printModel=args.printModel
        )
        if satisfiable_round1 and R_star != 0:
            pass  # sys.stderr.write("WARNING!!!!!!!!!!!!!!!! Actually sat.\n")
        elif not satisfiable_round1 and R_star == 0:
            pass  # sys.stderr.write("WARNING!!!!!!!!!!!!!!!  Wrong model Maybe unsat!\n")
        else:
            pass
    ...
    if args.verify:
        ...
        if verified and R_star != 0:
            ...
            pass  # sys.stderr.write("WARNING!!!!!!!!!!!!!!!  Wrong model !\n")
        else:
            pass
    ...
    if args.verify2:
        ...
        reload(
            foo_verify
        )  # necessary because name 'foo' now still points to foo_square
        verify_res = (
            foo_verify.R(*X_star) if foo_verify.dim == 1 else foo_verify.R(*(X_star))
        )

        if verify_res == 0 and R_star != 0:
            sys.stderr.write(
                "WARNING from verify2 (using include/R_verify/xsat.h) !!!!!!!!!!!!!!!! Actually sat.\n"
            )
        elif verify_res != 0 and R_star == 0:
            sys.stderr.write(
                "WARNING from verify2  (using include/R_verify/xsat.h) !!!!!!!!!!!!!!!  Wrong model ! \n"
            )
        else:
            pass
