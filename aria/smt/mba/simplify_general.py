...
class GeneralSimplifier:
    ...
    def __get_linear_combination(self, node) -> List[int]:
        ...
        for comb in combinations:
            ...
            self.__subtract_coefficient(
                resultVector, coeff, index, comb, groupsizes[comb[0]]
            )
    ...
    def __get_product_linear_combination(self, node):
        assert len(node.children) == 2 or (
            len(node.children) == 3 and node.children[0].type == NodeType.CONSTANT
        )
        ...
        res = np.zeros(
            2 ** (2 * self.__vnumber - 1) + 2 ** (self.__vnumber - 1), dtype=np.uint64
        )
        baselen = 2**self.__vnumber

        if len(node.children) == 3 and node.children[0].type == NodeType.CONSTANT:
            for i in range(len(linCombs[0])):
                linCombs[0][i] *= node.children[0].constant
        ...
        for b in range(baselen):
            ...
            for a in range(b + 1, baselen):
                res[idx] = self.__mod_red(
                    linCombs[0][b] * linCombs[1][a] + linCombs[0][a] * linCombs[1][b]
                )
                idx += 1
        assert idx == res.size
    ...
    def __get_power_linear_combination(self, node):
        ...
        if coeff != 1:
            for i in range(res.size):
                res[i] = self.__mod_red(int(res[i]) * coeff)
    ...
    def __try_simplify_sum_nonlinear_part(self, node) -> bool:
            ...
            for a in range(b + 1, baselen):
                if res[idx] != 0:
                    if res[idx] != 1:
                        simpl += str(res[idx]) + "*"
                    simpl += (
                        self.__get_basis_expression(b)
                        + "*"
                        + self.__get_basis_expression(a)
                        + "+"
                    )
                idx += 1
        assert idx == res.size
        ...
        for i in sorted(indices[1:], reverse=True):
            del node.children[i]
        if simpl != "":
            node.children[indices[0]] = parse(
                simpl, self.__bitCount, self.__modRed, True, True
            )
        else:
    ...
    def __is_candidate_for_simplification_in_sum(self, node) -> bool:
        if node.type != NodeType.PRODUCT and node.type != NodeType.POWER:
            return False
        if node.state != NodeState.NONLINEAR:
            return False

        if len(node.children) > 2:
            if len(node.children) > 3 or node.children[0].type != NodeType.CONSTANT:
                return False

        if node.type == NodeType.POWER:
            # TODO: Handle higher exponents.
            if (
                node.children[1].type != NodeType.CONSTANT
                or node.children[1].constant != 2
            ):
                return False
            if (
                node.children[0].state == NodeState.NONLINEAR
                or node.children[0].state == NodeState.MIXED
            ):
                return False
        else:
            if (
                node.children[0].type == NodeType.CONSTANT
                and node.children[1].type == NodeType.POWER
            ):
                if len(node.children) > 2:
                    return False
                return self.__is_candidate_for_simplification_in_sum(node.children[1])
            if node.has_nonlinear_child():
                return False
    ...
    def __get_indices_of_simple_nonlinear_products_in_sum(self, node) -> List[int]:
        ...
        if node.linearEnd >= len(node.children) - 1:
            return []
    ...
    def __simplify_nonlinear_subexpression_linear_part(self, node) -> bool:
        ...
        simpl = simplify_linear_mba(
            subexpr, self.__bitCount, False, False, self.__modRed
        )
    ...
    # to the given node.
    def __simplify_nonlinear_subexpression_step(
        self, node, parent, noRefactor: bool, noSubst: bool
    ) -> bool:
        ...
        if not noRefactor:
            ...
            if ch:
                ...
                for child in node.children:
                    self.__simplify_subexpression(child, node)
                node.refine()
                node.mark_linear()

        if not noSubst and node.type != NodeType.NEGATION:
            if self.__simplify_via_substitution(node):
                changed = True

        return changed

    # Simplify the nonlinear subexpression corresponding to the given node.
    def __simplify_nonlinear_subexpression(
        self, node, parent, noRefactor: bool = False, noSubst: bool = False
    ) -> bool:
        prev = {str(node)}
        changed = False
        ...
        for i in range(self.__MAX_IT):
            ch = self.__simplify_nonlinear_subexpression_step(
                node, parent, noRefactor, noSubst
            )
            if ch:
                changed = True
    ...
    def __simplify_linear_subexpression(self, node) -> bool:
        ...
        if changed:
            node.copy(parse(simpl, self.__bitCount, self.__modRed, True, True))
        return changed
    ...
    # Try to simplify the expression further by substituting subexpressions
    # equal to the given node by a variable.
    def __simplify_via_substitution_of_nodes(
        self, node, nodes, onlyFullMatch: bool
    ) -> bool:
        ...
        # refinement.
        if node.refine_after_substitution():
            node.refine()
    ...
    def __simplify_via_substitution_for_index(self, node, nodes, index: int) -> bool:
        ...
        changed = False
        if self.__simplify_via_substitution_of_nodes(node, sel, False):
            changed = True
        if self.__simplify_via_substitution_of_nodes(node, sel, True):
            changed = True
    ...
    def __simplify_via_substitution(self, node) -> bool:
        ...
        for i in range(1, 2 ** len(nodes)):
            ...
            if len(nodes) > 5 and popcount(i) > 3:
                continue
            if len(nodes) > 9 and popcount(i) > 2:
                continue

            if self.__simplify_via_substitution_for_index(node, nodes, i):
                changed = True

        return changed

    # Simplify the subexpression corresponding to the given node.
    def __simplify_subexpression(
        self, node, parent, noRefactor: bool = False, noSubst: bool = False
    ) -> bool:
        if node.is_linear():
    ...
    def _simplify(self, expr: str, returnDict):
        ...
        if root == None:
            sys.exit("Error: Could not parse expression!")
    ...
    def simplify(self, expr: str, useZ3: bool = False) -> str:
        ...
        if len(returnDict.values()) == 0:
            sys.exit("No simplification result!")
        ...
        if useZ3 and not self.__verify_using_z3(expr, simpl):
            sys.exit(
                "Error in simplification! Simplified expression is not proved equivalent to original one!"
            )
...
def print_usage():
    ...
    print(
        "    -m:    enable a reduction of all constants modulo 2**b where b is the bit count"
    )
    print(
        "    -v:    specify a bit count for verification for nonlinear input (default: no verification)"
    )
...
if __name__ == "__main__":
    ...
    for expr in args.exprs:
        ...
        simpl = simplify_mba(
            expr, args.bitCount, args.useZ3, args.modRed, args.verifyBitCount
        )
