...
class SMTConverter:
    ...
    def _convert_constant(self, const: Constant) -> z3.ExprRef:
        ...
        if const.theory == Theory.BV:
            # Default 32-bit
            return z3.BitVecVal(const.value, 32, self.context)
        if const.theory == Theory.STRING:
