...
class OutputParser:
    ...
    def parse_positive_example(self):
        ...
        soundness_code_lines = [
            line for line in soundness_code_lines if "copy" not in line
        ]
        soundness_code_lines = [
            line for line in soundness_code_lines if "minimize" not in line
        ]
        soundness_code_lines = ["\t" + line.strip() for line in soundness_code_lines]

        property_call = soundness_code_lines[-2].replace(
            "obtained_property", "property"
        )
    ...
    def parse_negative_example_precision(self):
        ...
        precision_code_lines = [
            line for line in precision_code_lines if "copy" not in line
        ]
        precision_code_lines = [
            line for line in precision_code_lines if "minimize" not in line
        ]
        ...
        if precision_code_lines[-2].find("(") >= 0:
            ...
            negative_example_code += "\n" + precision_code_lines[-2].replace(
                "out_3", "out"
            )
            negative_example_code += "\n\tassert !out;"
