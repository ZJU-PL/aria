...
from aria.fol.miniprover.language import (
    And, ForAll, Function, Implies, Not, Or, Predicate, ThereExists, Variable
)
...
def parse(tokens):  # pylint: disable=too-many-return-statements,too-many-branches,too-many-statements
...
def typecheck_formula(formula):  # pylint: disable=too-many-return-statements
    ...
    if isinstance(formula, ForAll):
        if not isinstance(formula.variable, Variable):
            raise InvalidInputError(f'Invalid bound variable in '
                                     f'FORALL quantifier: {formula.variable}.')
        typecheck_formula(formula.formula)
        return
    if isinstance(formula, ThereExists):
        if not isinstance(formula.variable, Variable):
            raise InvalidInputError(f'Invalid bound variable in '
                                    f'exists quantifier: {formula.variable}.')
        typecheck_formula(formula.formula)
        return
