...
Variable : a class to define Variable, for use in datalog queries. (defined in py_parser)
...
def assert_fact(predicate_name, *args):
    ...
    + py_parser.Literal.make(predicate_name, [py_parser.Expression._pyD_for(arg) for arg in args])
...
def _predicate(func):
    ...
    py_engine.Python_resolvers[func.__name__ + '/' + str(arity)] = func
    return func
...
def load(code):
    ...
    return py_parser.load(code, newglobals=newglobals)

def ask(code):
    """returns the result of the query contained in the code string"""
    return py_parser.ask(code)
...
class Classe(object):
    ...
    def __call__(self, *arguments, **keyargs):
        for a in arguments: 
            if isinstance(a, py_parser.Expression):
                assert not keyargs, "Sorry, key word arguments are not supported yet" #TODO
                return py_parser.Operation(self.cls, '(', arguments) 
        ...
        if inspect.isgeneratorfunction(arg):
            ...
            def atomized(*arguments, **keyargs):
                ...
                for a in arguments: 
                    if isinstance(a, py_parser.Expression):
                        assert not keyargs, "Sorry, key word arguments are not supported yet" #TODO
                        return py_parser.Operation(arg, '(', arguments) 
    ...
    try:
        ...
        for arg in set(args + ATOMS):
            ...
            if words[0] in b: # if it 's a builtin
                ...
                locals_[arg] = py_parser.Term(arg)
    finally:
        del stack
...
def variables(n):
    ...
    return [py_parser.Term('??') for i in range(n)]

Variable = py_parser.Term
Answer = py_parser.Answer
...
class metaMixin(type):
    ...
    def __init__(cls, name, bases, dct):
        ...
        def _getattr(self, attribute):
            ...
            if not attribute == '__iter__' and not attribute.startswith('_sa_'):
                ...
                terms = (py_parser.Term('_pyD_class', forced_type='constant'), self, py_parser.Term("X")) #prefixed
                literal = py_parser.Literal.make(predicate_name, terms) #TODO predicate_name[:-2]
    ...
    def __getattr__(cls, method):
        ...
        if cls in ('Mixin', 'metaMixin') or method in (
            ...
            raise AttributeError
        return py_parser.Term("%s.%s" % (cls.__name__, method))

    def pyDatalog_search(cls, literal):
        """Called by py_engine to resolve a prefixed literal for a subclass of Mixin."""
        terms = literal.terms
        ...
        if len(terms)==3: #prefixed
            ...
            if X.is_const():
                ...
                if not Y.is_const() or not operator or py_engine.compare(Y1,operator,Y.id):
                ...
                if cls.session:
                    ...
                    check_attribute(cls)
                    if Y.is_const():
                        q = q.filter(py_engine.compare(getattr(cls, attr_name), operator, Y.id))
                    for r in q:
...
def __init__(self):
    if not self.__class__.has_SQLAlchemy:
        for cls in self.__class__.__mro__:
            if cls.__name__ in py_engine.Class_dict and cls not in (Mixin, object):
