...
from aria.pyomt.result import OptimizationResult, maxsmt_result_tuple
...
class MaxSMTSolver:
    ...
    def __init__(self, algorithm: str = "core-guided", solver_name: str = "z3"):
        ...
        elif algorithm in ("core-guided", MaxSMTAlgorithm.CORE_GUIDED):
            self.solver = CoreGuidedSolver(solver_name)
        elif algorithm in ("ihs", MaxSMTAlgorithm.IHS):
            self.solver = ImplicitHittingSetSolver(solver_name)
        elif algorithm in ("local-search", MaxSMTAlgorithm.LOCAL_SEARCH):
