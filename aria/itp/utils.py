...
from aria.itp.kernel import is_proof
...
def generate(sort: smt.SortRef, pred=None) -> Generator[smt.ExprRef, None, None]:
    ...
    if pred is not None:
        s.add(pred(x))
    if sort in itp.notation.wf.methods:
        s.add(itp.notation.wf(x))
...
def free_vars(t: smt.ExprRef) -> set[smt.ExprRef]:
    ...
    while todo:
        ...
        if smt.is_var(t) or is_value(t) or smt.is_constructor(t):
            continue
        if smt.is_const(t) and t.decl() not in itp.kernel.defns:
            fvs.add(t)
...
def lemma_db() -> dict[str, itp.kernel.Proof]:
    """Scan all modules for Proof objects and return a dictionary of them."""
    db: dict[str, itp.kernel.Proof] = {}
...
def search_expr(
    ...
    >>> x,z = smt.Ints("x z")
    >>> search_expr(z + 0, {\
        "thm1": itp.prove(smt.ForAll([x], x + 0 == x)),\
        "thm2" : itp.prove(z + 0 == z),\
        "thm3" : itp.prove(smt.ForAll([x], x + 1 == 1 + x)),\
        "thm4" : itp.prove(smt.BoolVal(True))})
    ...
    for name, pf in pfs.items():
        try:  # try to convert to rewrite rule
            rule = itp.rewrite.rewrite_of_expr(pf.thm)
        ...
        except itp.rewrite.RewriteRuleException as _:
            pass
...
def search_decl(
    ...
    for name, pf in db.items():
        if itp.kernel.is_proof(pf) and f in itp.utils.decls(pf.thm):
            found[(name, pf)] = ()
    return found
