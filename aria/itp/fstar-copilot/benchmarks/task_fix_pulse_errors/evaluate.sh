#!/usr/bin/env bash
# Evaluator for task_fix_pulse_errors
set -euo pipefail
source "$(dirname "$0")/../lib/common.sh"

FILE="$WORKSPACE/BrokenPulse.fst"
...
# Function signatures preserved (5 points)
preserved=0
for fn in "is_empty" "read_length" "clear_buffer" "advance_head" "read_at_head"; do
    if grep -qE "^fn\s+$fn\b" "$FILE"; then
        preserved=$((preserved + 1))
    fi
done
record_score "$TASK_NAME" "signatures-preserved" "$preserved" 5
