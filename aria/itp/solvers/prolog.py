    ...
    for stmt in t.children:
        if stmt.data == "fact":
            ...
            if len(vs) == 0:
                ...
                clauses.append(itp.QForAll(vs, smt.And(*body), head))
...
def prolog(
    ...
    >>> import itprag.theories.nat as nat
    >>> x,y,z = smt.Consts("x y z", nat.Nat)
    >>> plus = smt.Function("plus", nat.Nat, nat.Nat, nat.Nat, smt.BoolSort())
    >>> rules = [\
        itp.QForAll([y], plus(nat.Z, y, y)),\
        itp.QForAll([x,y,z], smt.Implies(\
            plus(x, y, z),\
        ...
        if len(goals) == 0:
            ...
            if smt.is_true(goal):
                ...
                for rule in rules:
                    rule = rule.freshen()
                    vs1 = vs + rule.vs
                    subst1 = itp.utils.unify(vs1, rule.conc, goal)
                    if subst1 is None:
                        continue
                    else:
