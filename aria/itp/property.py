...
class GenericProof:
    ...
    It is similar to an axiom schema is some respects (in usage it takes in python data and outputs a `itp.Proof`) except that one must register
    ...
    >>> R = smt.RealSort()
    >>> obvious.register(R, itp.prove(smt.ForAll([x(R)], x(R) == x(R))))
    """
    ...
    def __call__(self, *args) -> itp.Proof:
        return self.data[args]

    def __getitem__(self, *args) -> itp.Proof:
        return self.data[args]

    def __contains__(self, *args) -> bool:
        return args in self.data

    def get(self, *args) -> Optional[itp.Proof]:
        return self.data.get(args)

    def lemma(self, *args, **kwargs):
        return self.register(*args, itp.kernel.prove(self.wrapped(*args), **kwargs))

    def register(self, *args):
        args, pf = args[:-1], args[-1]
        if not itp.kernel.is_proof(pf):
            raise ValueError("Not a proof", pf)
        formula = self.wrapped(*args)
        if not itp.utils.alpha_eq(formula, pf.thm):
...
def assoc(f: smt.FuncDeclRef):
    ...
    return itp.QForAll([x, y, z], f(f(x, y), z) == f(x, f(y, z)))


x, y, z = smt.Ints("x y z")
assoc.register(
    (x + y).decl(), itp.prove(smt.ForAll([x, y, z], (x + y) + z == x + (y + z)))
)
assoc.register(
    (x * y).decl(), itp.prove(smt.ForAll([x, y, z], (x * y) * z == x * (y * z)))
)
...
def comm(f: smt.FuncDeclRef):
    ...
    return itp.QForAll([x, y], f(x, y) == f(y, x))
...
comm.register((x + y).decl(), itp.prove(smt.ForAll([x, y], x + y == y + x)))


def assoc_comm(f: smt.FuncDeclRef) -> itp.Proof:
    x, y, z = smt.Consts("x y z", f.domain(0))
    return itp.prove(
        itp.QForAll([x, y, z], f(x, (y, z)) == f(y, f(x, z))), by=[assoc[f], comm[f]]
    )
...
def idem(f: smt.FuncDeclRef):
    T = f.domain(0)
    x = smt.Const("x", T)
    return itp.QForAll([x], f(x, x) == x)


a, b = smt.Bools("a b")
idem.register(smt.And(a, a).decl(), itp.prove(smt.ForAll([a], smt.And(a, a) == a)))
