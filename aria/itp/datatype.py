...
def datatype_call(
    ...
    >>> Point = itp.Struct("Point", ("x", smt.IntSort()), ("y", smt.IntSort()))
    >>> Point(1,2)
    Point(1, 2)
    >>> Point(y=2, x=1)
    Point(1, 2)
...
def datatype_replace(self: smt.DatatypeRef, **kwargs: smt.ExprRef) -> smt.DatatypeRef:
    ...
    >>> Point = itp.Struct("Point", ("x", smt.RealSort()), ("y", smt.RealSort()))
...
def datatype_match_(x, *cases, default=None):
    ...
    >>> import aria.itp.theories.list as list_
...
rel = itp.notation.SortDispatch(name="rel")
