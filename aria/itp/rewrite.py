...
def simp1(t: smt.ExprRef) -> smt.ExprRef:
    ...
    G = smt.Goal()
    for v in itp.kernel.defns.values():
        G.add(v.ax.thm)
    G.add(expr == t)
...
def unfold(e: smt.ExprRef, decls=None, trace=None) -> smt.ExprRef:
    """
    Do a single unfold sweep, unfolding definitions defined by `itp.define`.
    ...
    if smt.is_app(e):
        ...
        defn = itp.kernel.defns.get(decl)
        ...
        vs, e1 = itp.utils.open_binder_unhygienic(e)
        if e.is_forall():
...
def full_simp(e: smt.ExprRef, trace=None) -> smt.ExprRef:
    ...
    >>> import aria.itp.theories.nat as nat
...
def simp(e: smt.ExprRef, trace=None, max_iter=3) -> smt.ExprRef:
    ...
    while True:
        ...
        if e1.eq(e):
            ...
            if trace is not None:
                trace.append(itp.kernel.prove(smt.Eq(e, e1)))
            e = e1
...
def def_eq(e1: smt.ExprRef, e2: smt.ExprRef, trace=None) -> bool:
    ...
    return itp.utils.alpha_eq(e1, e2)
    """
...
class RewriteRule(NamedTuple):
    ...
    lhs: smt.ExprRef
    rhs: smt.ExprRef
    pf: Optional[itp.kernel.Proof] = None
...
def rewrite_of_expr(
    ...
    if isinstance(thm, itp.kernel.Proof):
        thm1 = thm.thm
    ...
    return RewriteRule(
        vs, lhs, rhs, pf=thm if isinstance(thm, itp.kernel.Proof) else None
    )
...
def rewrite_slow(
    ...
    >>> unit = itp.prove(smt.ForAll([x], x + 0 == x))
    >>> x + 0 + 0 + 0 + 0
    x + 0 + 0 + 0 + 0
...
def rule_of_expr(pf_or_thm: smt.ExprRef | itp.kernel.Proof) -> Rule:
    ...
    if isinstance(pf_or_thm, smt.ExprRef):
        thm = pf_or_thm
        pf = None
    elif itp.kernel.is_proof(pf_or_thm):
        pf = pf_or_thm
        thm = pf.thm
    else:
...
def backward_rule(
    ...
    """
    subst = itp.utils.pmatch(r.vs, r.conc, tgt)
    if subst is not None:
...
def forward_rule(r: Rule, tgt: smt.BoolRef):
    ...
    Apply a rule to a target term.
    """
    subst = itp.utils.pmatch(r.vs, r.hyp, tgt)
    if subst is not None:
...
def apply_horn(thm: smt.BoolRef, horn: smt.BoolRef) -> smt.BoolRef:
    ...
    while True:
        if smt.is_implies(pat):
            ...
            break
    return itp.utils.z3_match(thm, pat)
