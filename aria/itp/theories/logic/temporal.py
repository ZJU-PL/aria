...
def TSort(sort):
    ...
    >>> _ = x + TLift(2.1)
    """
    S = itp.NewType(f"T_{sort.name()}", smt.ArraySort(smt.IntSort(), sort))
    x, y = smt.Consts("x y", S)
    t = smt.Int("t")
    # itp.notation.add.register(S, lift_binop(S, operator.add))
    if sort == smt.IntSort() or sort == smt.RealSort() or sort in itp.notation.add:
        itp.notation.add.define([x, y], S(smt.Lambda([t], x.val[t] + y.val[t])))
    itp.notation.sub.register(S, lift_binop(S, operator.sub))
    itp.notation.mul.register(S, lift_binop(S, operator.mul))
    itp.notation.div.register(S, lift_binop(S, operator.truediv))
    itp.notation.and_.register(S, lift_binop(S, operator.and_))
    itp.notation.or_.register(S, lift_binop(S, operator.or_))
    itp.notation.invert.register(S, Not)
    itp.notation.eq.register(S, Eq)
    itp.notation.ne.register(S, NEq)
    itp.notation.getitem.register(S, lambda x, y: x.val[y])
    return S

    # itp.notation.eq.register(S, lift(operator.eq))
...
t = smt.Int("t")
itp.notation.ge.define([x, y], TBool(smt.Lambda([t], x.val[t] >= y.val[t])))
itp.notation.le.define([x, y], TBool(smt.Lambda([t], x.val[t] <= y.val[t])))
...
def Always(x: smt.DatatypeRef, vs=None) -> smt.DatatypeRef:
    ...
    >>> _ = itp.prove(smt.Not(Valid(Always(s))))
    >>> _ = itp.prove(Valid(Always(Next(s))))
    """
    assert x.sort() == TBool
    if vs is not None:
    ...
    dt = smt.FreshInt("dt")
    S = x.sort()
    return S(smt.Lambda([t], itp.QForAll([dt], dt >= 0, x.val[t + dt])))
...
def Eventually(x):
    ...
    dt = smt.FreshInt("dt")
    S = x.sort()
    return S(smt.Lambda([t], itp.QExists([dt], dt >= 0, x.val[t + dt])))
...
tnot = itp.define("tnot", [p], Not(p))
tand = itp.define("tand", [p, q], And(p, q))
tor = itp.define("tor", [p, q], Or(p, q))
timpl = itp.define("timpl", [p, q], Implies(p, q))
implies = timpl
eventually = itp.define("eventually", [p], Eventually(p))
always = itp.define("always", [p], Always(p))
bnext = itp.define("next", [p], Next(p))
beq = itp.define("beq", [p, q], Eq(p, q))
valid = itp.define("valid", [p], Valid(p))
valid_and = itp.prove(
...
inext = itp.define("inext", [x], Next(x))
if_int = itp.define("if_int", [p, x, y], If(p, x, y))
x = smt.Int("x")
tint = itp.define("tint", [x], TInt(smt.K(smt.IntSort(), x)))
# annoyingly polymorphic
# tif = itp.define("tif", [p, q, r], If(p, q, r))
# teq
# tiff = itp.define("tiff", [p, q], smt.Eq(p, q))
