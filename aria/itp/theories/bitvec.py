...
def BitVecSort(N):
    ...
    S.bvadd_comm = itp.prove(smt.ForAll([x, y], x + y == y + x))
    S.bvadd_assoc = itp.prove(smt.ForAll([x, y, z], (x + y) + z == x + (y + z)))
    S.bvadd_id = itp.prove(smt.ForAll([x], x + zero == x))
    S.bvadd_neg = itp.prove(smt.ForAll([x], x + (-x) == zero))

    S.bvsub_self = itp.prove(smt.ForAll([x], x - x == zero))
    S.bvsub_def = itp.prove(smt.ForAll([x, y], x - y == x + (-y)))

    S.bvmul_comm = itp.prove(smt.ForAll([x, y], x * y == y * x))
    S.bvmul_assoc = itp.prove(smt.ForAll([x, y, z], (x * y) * z == x * (y * z)))
    S.bvmul_id = itp.prove(smt.ForAll([x], x * smt.BitVecVal(1, N) == x))
    S.bvmul_zero = itp.prove(smt.ForAll([x], x * zero == zero))

    S.bvand_comm = itp.prove(smt.ForAll([x, y], x & y == y & x))
    S.bvand_assoc = itp.prove(smt.ForAll([x, y, z], (x & y) & z == x & (y & z)))
    S.bvand_id = itp.prove(smt.ForAll([x], x & smt.BitVecVal(-1, N) == x))
    S.bvand_zero = itp.prove(smt.ForAll([x], x & zero == zero))

    S.bvor_comm = itp.prove(smt.ForAll([x, y], x | y == y | x))
    S.bvor_assoc = itp.prove(smt.ForAll([x, y, z], (x | y) | z == x | (y | z)))
    S.bvor_id = itp.prove(smt.ForAll([x], x | zero == x))
    S.bvor_neg = itp.prove(smt.ForAll([x], x | ~x == smt.BitVecVal(-1, N)))

    S.bvxor_comm = itp.prove(smt.ForAll([x, y], x ^ y == y ^ x))
    S.bvxor_assoc = itp.prove(smt.ForAll([x, y, z], (x ^ y) ^ z == x ^ (y ^ z)))
    S.bvxor_id = itp.prove(smt.ForAll([x], x ^ zero == x))
    S.bvxor_self = itp.prove(smt.ForAll([x], x ^ x == zero))

    S.bvshl_zero = itp.prove(smt.ForAll([x], x << zero == x))
    S.bvshr_zero = itp.prove(smt.ForAll([x], smt.LShR(x, zero) == x))

    # Bitwise simplification rules
    S.bvand_self = itp.prove(smt.ForAll([x], x & x == x))
    S.bvor_self = itp.prove(smt.ForAll([x], x | x == x))
    S.bvxor_zero = itp.prove(smt.ForAll([x], x ^ zero == x))
    S.bvnot_self = itp.prove(smt.ForAll([x], ~x == -x - 1))

    # Rules for shifting and rotating
    S.bvshl_self = itp.prove(
        smt.ForAll([x, y], x << y == x * (one << y))
    )  # Left shift as multiplication
    # bvshr_self = itp.prove(smt.ForAll([x, y], smt.LShR(x, y) == x / (one << y)))  # Logical right shift as division
    # bvashr_self = itp.prove(smt.ForAll([x, y], smt.AShr(x, y) == smt.If(x >> 31 == 0, smt.LShR(x, y), ~smt.LShR(~x, y))))  # Arithmetic right shift rule

    # Simplification with negation and subtraction
    S.bvsub_zero = itp.prove(smt.ForAll([x], x - zero == x))
    S.bvsub_id = itp.prove(smt.ForAll([x], zero - x == -x))
    S.bvadd_sub = itp.prove(smt.ForAll([x, y], x + (-y) == x - y))
    S.bvsub_add = itp.prove(smt.ForAll([x, y], x - (-y) == x + y))

    # Bitwise AND, OR, and XOR with constants
    S.bvand_allones = itp.prove(smt.ForAll([x], x & smt.BitVecVal(-1, N) == x))
    S.bvor_allzeros = itp.prove(smt.ForAll([x], x | zero == x))
    S.bvxor_allzeros = itp.prove(smt.ForAll([x], x ^ zero == x))

    # Distribution and absorption laws
    S.bvand_or = itp.prove(smt.ForAll([x, y, z], x & (y | z) == (x & y) | (x & z)))
    S.bvor_and = itp.prove(smt.ForAll([x, y, z], x | (y & z) == (x | y) & (x | z)))
    S.bvand_absorb = itp.prove(smt.ForAll([x, y], x & (x | y) == x))
    S.bvor_absorb = itp.prove(smt.ForAll([x, y], x | (x & y) == x))

    # Shifting rules with zero and identity
    S.bvshl_zero_shift = itp.prove(smt.ForAll([x], x << zero == x))
    S.bvshr_zero_shift = itp.prove(smt.ForAll([x], smt.LShR(x, zero) == x))
    # bvashr_zero_shift = itp.prove(smt.ForAll([x], smt.AShr(x, zero) == x))  # Arithmetic right shift by zero is identity
    S.bvshl_allzeros = itp.prove(smt.ForAll([y], zero << y == zero))
    S.bvshr_allzeros = itp.prove(smt.ForAll([y], smt.LShR(zero, y) == zero))
    # bvashr_allzeros = itp.prove(smt.ForAll([y], smt.AShr(zero, y) == zero))  # Arithmetic right shift of zero is zero

    # Additional rules for combining operations
    # bvadd_and = itp.prove(smt.ForAll([x, y, z], (x & y) + (x & z) == x & (y + z)))  # AND distribution over addition
    S.bvor_and_not = itp.prove(smt.ForAll([x, y], (x & y) | (x & ~y) == x))
    # bvxor_and_not = itp.prove(smt.ForAll([x, y], (x & y) ^ (x & ~y) == y))  # Distribution of XOR and AND with negation

    # Properties involving shifts and bit manipulations
    S.bvshl_and = itp.prove(smt.ForAll([x, y, z], (x & y) << z == (x << z) & (y << z)))
    S.bvshr_and = itp.prove(
...
BitVecN = itp.NewType("BitVecN", seq.Seq(BV1))
"""
...
to_int = itp.notation.to_int.define(
    [x],
    smt.If(
        smt.Length(x.val) == 0,
        smt.IntVal(0),
...
def select64(outsize: int) -> smt.FuncDeclRef:
    ...
    return itp.define("select64", [a, addr], SelectConcat(a, addr, outsize))
