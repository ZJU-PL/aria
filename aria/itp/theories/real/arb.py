...
import aria.itp.theories.real as real
...
def flint_eq_ax_unsafe(lhs, rhs):
    ...
    return itp.axiom(smt.Eq(lhs, rhs), by="flint")


def arb_lt_ax(lhs: smt.ArithRef, rhs: smt.ArithRef) -> itp.kernel.Proof:
    """
    ...
    if interp_flint(lhs, {}) < (interp_flint(rhs, {})):
        return itp.axiom(lhs < rhs, by="flint")  # type: ignore
    else:
        raise ValueError(
...
def arb_le_ax(lhs0: smt.ArithRef, rhs: smt.ArithRef) -> itp.kernel.Proof:
    """
...
def arb_gt(lhs, rhs):
    itp.prove(lhs > rhs, by=[arb_lt_ax(rhs, lhs)])


def arb_ge(lhs, rhs):
    itp.prove(lhs >= rhs, by=[arb_le_ax(rhs, lhs)])


def sin_bnd(x: Arb) -> itp.kernel.Proof:
    """
    >>> sin_bnd(arb(0))
    |- ForAll(x,
        Implies(And(0 - 0 <= x, x <= 0 + 0),
                And(0 - 0 <= sin(x), sin(x) <= 0 + 0)))
    >>> itp.prove(real.sin(0) <= 2, by=[sin_bnd(arb(0,0.1))(smt.RealVal(0))])
    |- sin(0) <= 2
    >>> itp.prove(real.sin(3.14) <= 0.02, by=[sin_bnd(arb(3.14, 0.01))])
    |- sin(157/50) <= 1/50
    """
...
def arb_bnd(arbf, z3f):
    """
    >>> itp.prove(real.cos(0) == 1, by=[cos_bnd(arb(0))])
    |- cos(0) == 1
    """

    def res(x: Arb) -> itp.kernel.Proof:
        xm, xr = z3_mid_rad_of_arb(x)
...
def bound_ax(e: smt.ArithRef) -> itp.kernel.Proof:
    lower, upper = bounds(e)
    return itp.axiom(smt.And(lower <= e, e <= upper), by="flint_bnd")


def flint_bnd(t: smt.ArithRef, env) -> itp.kernel.Proof:
    ...
    if len(env) > 0:
        ...
        return itp.axiom(smt.And(mid - rad <= t, t <= mid + rad), by="flint_eval")
