...
zeroF_is_positive = itp.prove(smt.fpIsPositive(smt.fpZero(F32, False)))
zeroF_is_negative = itp.prove(smt.fpIsNegative(smt.fpZero(F32, True)))
...
# Actually, let's just complete it to the identity function and use itp.define?
round_def = itp.axiom(
    itp.QForAll(
        [m, x],
        NoOverflow(m, x, F32),
...
bounded_real_no_overflow = itp.prove(
    # itp.QForAll( # quantifier form doesn't pass?
    #    [m, x],
    smt.Implies(in_range(x), NoOverflow(m, x, F32))
    # )
)

bounded_no_overflow = itp.axiom(itp.QForAll([m, x], in_range(x), no_overflow(m, x)))
...
round_monotonic = itp.axiom(itp.QForAll([x, y], x <= y, round(m, x) <= round(m, y)))
round_idem = itp.axiom(itp.QForAll([x, m1, m2], round(m1, round(m2, x)) == round(m2, x)))

round_to_real = itp.axiom(
    itp.QForAll([z, m], IsFinite(z), round(m, smt.fpToReal(z)) == smt.fpToReal(z))
)

round_down_le = itp.axiom(smt.ForAll([x], round(smt.RTN(), x) <= x))
round_up_ge = itp.axiom(smt.ForAll([x], round(smt.RTP(), x) >= x))
...
z = smt.Const("z", F64)
max_real_thm = itp.prove(
    itp.QForAll([z], IsFinite(z), z <= smt.fpRealToFP(smt.RNE(), max64, F64))
)
min_float64 = itp.prove(
    itp.QForAll([z], IsFinite(z), z >= smt.fpRealToFP(smt.RNE(), -max64, F64))
)
"""
>>> assert True
