        ...
        if res == z3.sat:
            ...
            for j in check_list:
                if results[j] is None and z3.is_true(
                    model.eval(cnt_list[j], model_completion=True)
                ):
                    results[j] = 1
        elif res == z3.unsat:
            results[i] = 0
...
def disjunctive_check_incremental_cached(
    ...
    if res == z3.unsat:
        ...
        new_check_list = []
        for i in check_list:
            if results[i] is None and z3.is_true(
...
def conjunctive_check_incremental(
    precond: z3.ExprRef, cnt_list: List[z3.ExprRef], alogorithm: int = 0
) -> List:
    """
    ...
    while queue:
        ...
        if (
            ...
            solver_result == z3.unsat
        ):  # conflicts within the predicates, need to split
            solver.pop()
...
def conjunctive_check(
    ...
    while queue:
        ...
        if (
            ...
            unsat_core_indices = {
                int(c.decl().name()) for c in solver_split.unsat_core()
            }
            unsat_set_indices = list(unsat_core_indices)
            sat_set_indices = [i for i in current_subset if i not in unsat_core_indices]
            if len(unsat_set_indices) == 1:
