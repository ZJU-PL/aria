...
from aria.efmc.verifytools.common.util import unique, error  # pylint: disable=unused-import
...
def convert_cpp_file_for_invgen(cpp_file, out_file):
    ...
    with NamedTemporaryFile(suffix=".cpp", delete=False) as processed_f:
        ...
        lines = (
            ["//Auto-generated by " + __file__ + " from " + cpp_file]
            + ["int NONDET;"]
            + list(lines)
        )
...
def run_invgen(cpp_file, main_routine):
    ...
    with NamedTemporaryFile(
        suffix=".front.out", delete=False
    ) as front_out, NamedTemporaryFile(
        suffix=".invgen.out", delete=False
    ) as invgen_out, NamedTemporaryFile(
        suffix=".pl", delete=False
    ) as transitions_f:
        ...
        try:
            ...
            if (
                outp[-2]
                == 'Fatal error: exception Failure("Push Negation failed! I am here!")'
            ):
