...
from aria.efmc.engines.polyhorn.Coefficient import Coefficient, Element, UnknownVariable
from aria.efmc.engines.polyhorn.Constant import Constant, Theorem
from aria.efmc.engines.polyhorn.Constraint import CoefficientConstraint
from aria.efmc.engines.polyhorn.Convertor import Polynomial, convert_general_string_to_poly
from aria.efmc.engines.polyhorn.DNF import DNF
from aria.efmc.engines.polyhorn.Farkas import Farkas
from aria.efmc.engines.polyhorn.Handelman import Handelman
from aria.efmc.engines.polyhorn.Putinar import Putinar
...
class PositiveModel:
    ...
    def __init__(
        ...
        theorem_name: str,
        get_SAT: bool = True,
        get_UNSAT: bool = False,
        get_strict: bool = False,
        degree_of_sat: int = 0,
        degree_of_nonstrict_unsat: int = 0,
        degree_of_strict_unsat: int = 0,
        ...
        for name in template_variables_name:
            self.template_variables.append(
                UnknownVariable(name=name, type_of_var="template_var")
            )
    ...
    def get_polynomial(self, poly_str: str, program_variables) -> Polynomial:
        ...
        """
        return convert_general_string_to_poly(
            poly_str, self.template_variables + program_variables, program_variables
        )
        ...
        for pair in self.paired_constraint:
            ...
            if theorem == Theorem.Farkas:
                ...
                model = Handelman(
                    ...
                    LHS=pair[0],
                    RHS=pair[1],
                    max_d_for_sat=self.degree_of_sat,
                    max_d_for_unsat=self.degree_of_nonstrict_unsat,
                )
    ...
    def create_smt_file(
        ...
        if (
            constant_heuristic
            and (self.get_SAT ^ self.get_UNSAT ^ self.get_strict)
            and (not (self.get_SAT and self.get_UNSAT and self.get_UNSAT))
        ):
            all_constraint = PositiveModel.remove_equality_constraints(all_constraint)
        if core_iteration_heuristic:
            all_constraint = self.core_iteration(
                all_constraint,
        ...
        f.write(
            solver_option
            + Solver.smt_declare_variable_phase(
