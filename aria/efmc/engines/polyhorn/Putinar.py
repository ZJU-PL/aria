...
from aria.efmc.engines.polyhorn.Constraint import CoefficientConstraint, PolynomialConstraint
from aria.efmc.engines.polyhorn.Polynomial import Polynomial
from aria.efmc.engines.polyhorn.Solver import Solver
from aria.efmc.engines.polyhorn.UnknownVariable import UnknownVariable
...
class Putinar:
    ...
    def __init__(
        ...
        max_d_for_sat=0,
        max_d_for_unsat=0,
        max_d_for_unsat_strict=0,
        degree_for_new_var=0,
    ):
    ...
    def get_lower_triangular_matrix(
        self, dim: int
    ) -> Tuple[List[List[Polynomial]], List[CoefficientConstraint]]:
        ...
        for i in range(dim):
            for j in range(i + 1):
                matrix[i][j] = Solver.get_variable_polynomial(
                    self.variables, f"l_{i}{j}", "generated_for_semidefinite_matrix"
                )
            constraints.append(
                CoefficientConstraint(matrix[i][i].monomials[0].coefficient, ">=")
            )
    ...
    def get_sum_of_square(
        self, max_d: int
    ) -> Tuple[Polynomial, List[CoefficientConstraint]]:
        ...
        """
        monoid = np.array(Putinar.get_monoids(self.variables, (max_d // 2))).reshape(
            1, -1
        )
        ...
        poly_with_more_degree = np.matmul(
            np.matmul(monoid, semi_definite_matrix), monoid.T
        )[0][0]
        monomials = []
        for i, monomial in enumerate(poly_with_more_degree.monomials):
            new_poly = Solver.get_variable_polynomial(
                self.variables, f"t_{i}", "eta_generated_for_sum_of_square"
    ...
    def get_poly_sum(
        ...
        if self.RHS.is_strict():
            ...
            all_constraints.append(
                CoefficientConstraint(new_var.monomials[0].coefficient, ">=")
            )
        ...
        if self.RHS.is_strict():
            all_constraints.append(
                CoefficientConstraint(strict_poly.monomials[0].coefficient, ">")
            )
    ...
    def get_SAT_constraint(self):
        ...
        return constraints + Solver.find_equality_constraint(
            polynomial_of_sum, self.RHS.polynomial
        )
    ...
    def get_UNSAT_constraint(self, need_strict=False):
        ...
        """
        if need_strict:
            return self.handel_strict(
                self.max_d_for_unsat_strict, self.degree_for_new_var
            )
    ...
    def handel_strict(
        self, max_d: int, power_of_new_var: int
        ...
        """
        new_variables = [
            UnknownVariable(name=f"w_{i + 1}", type_of_var="generated_for_strict_unsat")
            for i in range(len(self.LHS))
        ]
        new_set_of_cons = []
        for left_constraint in self.LHS:
            new_set_of_cons.append(
                PolynomialConstraint(
                    left_constraint.polynomial.add_variables(new_variables),
        ...
        for j, left_constraint in enumerate(new_set_of_cons):
            if left_constraint.is_strict():
                ...
                all_constraints.append(
                    Solver.find_equality_constraint(poly_of_var_to_power, poly_sum)
                )
        return all_constraints

    @staticmethod
    def get_general_template(
        all_variables: List[UnknownVariable], all_monoids: List[Polynomial]
    ) -> List[Polynomial]:
        ...
        return np.matmul(
            np.array(new_unknown_variables).reshape(1, -1),
            np.array(all_monoids).reshape(-1, 1),
        )[0][0]
