...
class Solver:
    ...
    def find_equality_constraint(
        ...
        """
        all_degree = set(LHS.dict_from_degrees_to_monomials.keys()).union(
    ...
    @staticmethod
    def get_constant_polynomial(
        variables: List[UnknownVariable], constant
    ) -> Polynomial:
        ...
        return Polynomial(
            ...
            [
                Monomial(
                    variables,
                    [0] * len(variables),
                    Coefficient([Element(constant, [])]),
                )
            ],
        )

    @staticmethod
    def get_variable_polynomial(
        variables: List[UnknownVariable], name: str, type_of_var: str = None
    ) -> Polynomial:
        ...
        return Polynomial(
            ...
            [
                Monomial(
                    variables,
                    [0] * len(variables),
                    Coefficient([Element("1", [new_variable])]),
                )
            ],
        )

    @staticmethod
    def get_degree_polynomial(
        variables: List[UnknownVariable], degrees: List[int]
    ) -> Polynomial:
        ...
        """
        return Polynomial(
            variables, [Monomial(variables, degrees, Coefficient([Element("1", [])]))]
        )

    @staticmethod
    def convert_constraints_to_smt_format(
        all_constraint: List[DNF], precondition, names: List[str] = None
    ) -> str:
        ...
        for i, constraint in enumerate(all_constraint):
            if names is None:
                smt_string = (
                    smt_string + f"(assert  {constraint.convert_to_preorder()} )\n"
                )
            else:
                smt_string = (
                    smt_string
        ...
        for constraint in precondition:
            if len(constraint) == 1:
                smt_string = (
                    smt_string + f"(assert  {constraint[0].convert_to_preorder()} )\n"
                )
    ...
    @staticmethod
    def smt_declare_variable_phase(
        all_constraint: List[DNF],
        real: bool = True,
        pre_variables: List[UnknownVariable] = [],
    ) -> str:
    ...
    def get_all_variable(
        all_constraints: List[DNF], pre_variables: List[UnknownVariable] = []
