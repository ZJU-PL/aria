...
from aria.efmc.engines.polyhorn.Polynomial import Monomial, Polynomial
...
def traverse(parse_tree, all_variables: List[UnknownVariable]) -> Polynomial:
    ...
    if parse_tree.data == "start":
        ...
        if len(parse_tree.children) == 1:
            ...
            return traverse(parse_tree.children[0], all_variables) + traverse(
                parse_tree.children[2], all_variables
            )
        elif parse_tree.children[1] == "-":
            return traverse(parse_tree.children[0], all_variables) - traverse(
                parse_tree.children[2], all_variables
        ...
        if len(parse_tree.children) == 1:
            ...
            return traverse(parse_tree.children[0], all_variables) * traverse(
                parse_tree.children[1], all_variables
            )
        ...
        if not parse_tree.children[0].__class__ is lark.Token:
            ...
            return Solver.get_constant_polynomial(
                all_variables, str(parse_tree.children[0])
            )
        else:
            deg = 1
            if len(parse_tree.children) > 1:
            ...
            degrees[
                find_index_of_variable(str(parse_tree.children[0]), all_variables)
            ] = deg
            return Solver.get_degree_polynomial(all_variables, degrees)


def convert_to_desired_poly(
    poly: Polynomial, program_variables: List[UnknownVariable]
) -> Polynomial:
    ...
    for monomial in poly.monomials:
        ...
        for i, var in enumerate(monomial.variables):
            if var in program_variables:
                degrees[find_index_of_variable(var.name, program_variables)] = (
                    monomial.degrees[i]
                )
            else:
...
def convert_general_string_to_poly(
    poly_text: str,
    ...
    return convert_to_desired_poly(
        traverse(parse_tree, all_variables), program_variables
    )
