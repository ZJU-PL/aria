...
def graphNum(s):
    ...
    try:
        ...
        raise GraphFormatError(f'Number expected: "{s}"') from exc
...
def vertex(G, v):
    ...
    if v in G:
        raise GraphFormatError(f'Duplicate vertex {v}')
    G[v] = {}

def edge(G,u,v,e):
    """Add edge e connecting vertices u and v in graph G.  Vertices must already be in G."""
    if u == v:
        raise GraphFormatError(f'Self-loop at {u}')
    if u not in G:
        raise GraphFormatError(f'Unexpected vertex {u} in edge to {v}')
    if v not in G:
        raise GraphFormatError(f'Unexpected vertex {v} in edge from {u}')
    G[u][v] = G[v][u] = e
...
def readEdgeList(lines):
    ...
    for line in filter(None,lines):
        ...
        if len(words) < 2 or len(words) > 3:
            raise GraphFormatError(f'Wrong number of words in edge list: "{line}"')
        if len(words) == 3 and words[1] != '-':
            raise GraphFormatError(f'Unrecognized edge type "{words[1]}" in edge list')
        u, v = words[0], words[-1]
        if u not in G:
            vertex(G, u)
        if v not in G:
...
def readNodeEdgeList(lines):
    ...
    def namedEdge(line):
        id = line
        if id in EdgeNames:
            raise GraphFormatError(f'Edge name "{id}" used twice in node edge list')
    ...
    for line in lines:
        if line.startswith('// '):
            try:
                ...
                raise GraphFormatError(f'Unrecognized section "{line[3:]}" in node edge list') from exc
        else:
            action(line)
...
def readGraphML(lines):
    ...
    def start_element(name,attrs):
        ...
        if len(context) == 1:
            if name != 'graphml':
                raise GraphFormatError(f'Unrecognized outer tag "{name}" in GraphML')
...
def readGraph6(str):
    ...
    nd = (n*(n-1)//2 + 5) // 6
    if len(data) != nd:
        raise GraphFormatError(f'Expected {n*(n-1)//2} bits but got {len(data)*6} in graph6')
...
def readLeda(lines):
    ...
    for i in range(m):
        ...
        if len(words) < 4:
            raise GraphFormatError(f'Too few fields in LEDA.GRAPH edge {i+1}')
        source = graphNum(words[0])
        target = graphNum(words[1])
        reversal = graphNum(words[2])
        if not reversal:
            raise GraphFormatError(f'Edge {i+1} is directed in LEDA.GRAPH')
        if source < target:
