...
IsSubset_Int_as_FuncDeclRef = z3.IsSubset(
    z3.EmptySet(z3.IntSort()), z3.EmptySet(z3.IntSort())
).decl()
...
def apply_bound_formula(bound_formula, args):
    ...
    if len(args) != arity:
        raise TypeError(
            "The arities of the bound formula and the arguments do not match."
        )
...
def transform_expression(expression, transformations):
    ...
    transformed_args = tuple(
        [transform_expression(arg, transformations) for arg in args]
    )
