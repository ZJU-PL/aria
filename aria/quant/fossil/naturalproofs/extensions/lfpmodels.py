...
from aria.quant.fossil.naturalproofs.uct import fgsort, boolsort, intsort
...
from aria.quant.fossil.naturalproofs.utils import apply_bound_formula
from aria.quant.fossil.naturalproofs.prover_utils import (
    instantiate,
    make_recdef_unfoldings,
)

from aria.quant.fossil.naturalproofs.extensions.finitemodel import FiniteModel
...
def rank_fcts():
    ...
    lseg_recdef = lseg(x, y) == If(x == y, True, If(x == nil, False, lseg(nxt(x), y)))
    lseg_rankdef = If(
        Or(x == y, x == nil),
        True,
    ...
    tree_rankdef = If(
        ...
        True,
        tree(x)
        == And(tree_rank(lft(x)) < tree_rank(x), tree_rank(rght(x)) < tree_rank(x)),
    )
    ...
    minr_recdef = If(
        ...
        minr(x) == 100,
        If(
            And(key(x) <= minr(lft(x)), key(x) <= minr(rght(x))),
            minr(x) == key(x),
            If(
                minr(lft(x)) <= minr(rght(x)),
                minr(x) == minr(lft(x)),
                minr(x) == minr(rght(x)),
            ),
        ),
    ...
    maxr_recdef = If(
        ...
        maxr(x) == -1,
        If(
            And(key(x) >= maxr(lft(x)), key(x) >= maxr(rght(x))),
            maxr(x) == key(x),
            If(
                maxr(lft(x)) >= maxr(rght(x)),
                maxr(x) == maxr(lft(x)),
                maxr(x) == maxr(rght(x)),
            ),
        ),
    ...
    bst_rankdef = If(
        x == nil,
        True,
        bst(x) == And(bst_rank(lft(x)) < bst_rank(x), bst_rank(rght(x)) < bst_rank(x)),
    )
    ...
    cyclic_recdef = cyclic(x) == If(x == nil, False, lseg(nxt(x), x))
    cyclic_rankdef = If(
        x == nil,
        True,
        cyclic(x)
        == And(
            lseg_rank(nxt(x), x) < cyclic_rank(x), cyclic_rank(nxt(x)) == cyclic_rank(x)
        ),
    )
    ...
    dag_recdef = dag(x) == If(x == nil, True, And(dag(lft(x)), dag(rght(x))))
    dag_rankdef = If(
        x == nil,
        True,
        dag(x) == And(dag_rank(lft(x)) < dag_rank(x), dag_rank(rght(x)) < dag_rank(x)),
    )
    ...
    dlst_recdef = dlst(x) == If(
        Or(x == nil, nxt(x) == nil), True, And(prv(nxt(x)) == x, dlst(nxt(x)))
    )
    ...
    even_lst_recdef = even_lst(x) == If(x == nil, True, even_lst(nxt(nxt(x))))
    even_lst_rankdef = If(
        x == nil, True, even_lst(x) == (even_lst_rank(nxt(nxt(x))) < even_lst_rank(x))
    )
    ...
    odd_lst_recdef = odd_lst(x) == If(
        x == nil, False, If(nxt(x) == nil, True, odd_lst(nxt(nxt(x))))
    )
    odd_lst_rankdef = If(
        nxt(x) == nil, True, odd_lst(x) == (odd_lst_rank(nxt(nxt(x))) < odd_lst_rank(x))
    )
    ...
    slst_recdef = slst(x) == If(
        Or(x == nil, nxt(x) == nil), True, And(key(x) <= key(nxt(x)), slst(nxt(x)))
    )
    slst_rankdef = If(
        Or(x == nil, nxt(x) == nil), True, slst(x) == (slst_rank(nxt(x)) < slst_rank(x))
    )
    ...
    slseg_recdef = slseg(x, y) == If(
        x == y, True, And(key(x) <= key(nxt(x)), slseg(nxt(x), y))
    )
    slseg_rankdef = If(
        x == y, True, slseg(x, y) == (slseg_rank(nxt(x), y) < slseg_rank(x, y))
    )
...
def rank_fcts_lightweight():
    ...
    lst_rank_def = (
        ...
        If(
            x == nil,
            lst_rank(x) == 0,
            If(lst(x), lst_rank(x) > lst_rank(nxt(x)), lst_rank(x) == -1),
        ),
    )
    ...
    lsegy_rank_def = (
        ...
        If(
            x == y,
            lsegy_rank(x) == 0,
            If(lsegy(x), lsegy_rank(x) > lsegy_rank(nxt(x)), lsegy_rank(x) == -1),
        ),
    )
    ...
    lsegz_rank_def = (
        ...
        If(
            x == y,
            lsegz_rank(x) == 0,
            If(lsegz(x), lsegz_rank(x) > lsegz_rank(nxt(x)), lsegz_rank(x) == -1),
        ),
    )
    ...
    lseg_rank_def = (
        ...
        If(
            ...
            If(
                lseg(x, y),
                lseg_rank(x, y) > lseg_rank(nxt(x), y),
                lseg_rank(x, y) == -1,
            ),
        ),
    )
    ...
    slst_rank_def = (
        ...
        If(
            x == nil,
            slst_rank(x) == 0,
            If(slst(x), slst_rank(x) > slst_rank(nxt(x)), slst_rank(x) == -1),
        ),
    )
    ...
    slseg_rank_def = (
        ...
        If(
            ...
            If(
                slseg(x, y),
                slseg_rank(x, y) > slseg_rank(nxt(x), y),
                slseg_rank(x, y) == -1,
            ),
        ),
    )
    ...
    dlst_rank_def = (
        ...
        If(
            x == nil,
            dlst_rank(x) == 0,
            If(dlst(x), dlst_rank(x) > dlst_rank(nxt(x)), dlst_rank(x) == -1),
        ),
    )
    ...
    sdlst_rank_def = (
        ...
        If(
            x == nil,
            sdlst_rank(x) == 0,
            If(sdlst(x), sdlst_rank(x) > sdlst_rank(nxt(x)), sdlst_rank(x) == -1),
        ),
    )
    # Record list
    ...
    rlst_rank_def = (
        ...
        If(
            x == nil,
            rlst_rank(x) == 0,
            If(rlst(x), rlst_rank(x) > rlst_rank(nxt(x)), rlst_rank(x) == -1),
        ),
    )
    # 'Odd' and 'Even' lists
    ...
    even_lst_rank_def = (
        ...
        If(
            ...
            If(
                ...
                If(
                    even_lst(x),
                    even_lst_rank(x) > even_lst_rank(nxt(nxt(x))),
                    even_lst_rank(x) == -1,
                ),
            ),
        ),
    ...
    odd_lst_rank_def = (
        ...
        If(
            ...
            If(
                ...
                If(
                    odd_lst(x),
                    odd_lst_rank(x) > odd_lst_rank(nxt(nxt(x))),
                    odd_lst_rank(x) == -1,
                ),
            ),
        ),
    ...
    tree_rank_def = (
        ...
        If(
            ...
            If(
                tree(x),
                And(
                    tree_rank(x) > tree_rank(lft(x)), tree_rank(x) > tree_rank(rght(x))
                ),
                tree_rank(x) == -1,
            ),
        ),
    ...
    bst_rank_def = (
        ...
        If(
            ...
            If(
                bst(x),
                And(bst_rank(x) > bst_rank(lft(x)), bst_rank(x) > bst_rank(rght(x))),
                bst_rank(x) == -1,
            ),
        ),
    )
    ...
    maxheap_rank_def = (
        ...
        If(
            ...
            If(
                maxheap(x),
                And(
                    maxheap_rank(x) > maxheap_rank(lft(x)),
    ...
    dag_rank_def = (
        ...
        If(
            ...
            If(
                dag(x),
                And(dag_rank(x) > dag_rank(lft(x)), dag_rank(x) > dag_rank(rght(x))),
                dag_rank(x) == -1,
            ),
        ),
    )
    ...
    tree_p_rank_def = (
        ...
        If(
            ...
            If(
                tree_p(x),
                And(
                    tree_p_rank(x) > tree_p_rank(lft(x)),
    ...
    reach_lr_rank = z3.Function(
        "reach_lr_rank", fgsort.z3sort, fgsort.z3sort, intsort.z3sort
    )
    reach_lr_rank_def = (
        (x, y),
        If(
            ...
            If(
                reach_lr(x, y),
                And(
                    reach_lr_rank(x, y) > reach_lr_rank(lft(x), y),
    ...
    reach_pgm_rank_def = (
        ...
        If(
            ...
            If(
                reach_pgm(x),
                reach_pgm_rank(x) > reach_pgm_rank(p(x)),
                reach_pgm_rank(x) == -1,
            ),
        ),
    )
...
def gen_lfp_model(size, annctx, invalid_formula=None):
    ...
    if all(value is None for value in rank_formulas.values()):
        raise Exception(
            ...
            "This should be one among: {}".format(
                ", ".join(key for key, value in rank_formulas.items() if value is None)
            )
        )
    # Axioms
    ...
    structural_constraints = (
        untagged_unfoldings
        | set(rankdef for rankdef in rank_formulas.values() if rankdef is not None)
        | axioms
    )
