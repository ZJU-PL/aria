...
import aria.quant.fossil.lemsynth.options as options
from aria.quant.fossil.lemsynth.induction_constraints import generate_pfp_constraint
from aria.quant.fossil.lemsynth.cvc4_compliance import cvc4_compliant_formula_sexpr
from aria.quant.fossil.lemsynth.ProcessStreamer import ProcessStreamer, Timeout
from aria.quant.fossil.lemsynth.utils import StopProposal

from aria.quant.fossil.naturalproofs.decl_api import (
    get_uct_signature,
    get_boolean_recursive_definitions,
    is_expr_fg_sort,
)
...
from aria.quant.fossil.naturalproofs.extensions.finitemodel import recover_value
from aria.quant.fossil.naturalproofs.extensions.finitemodel import FiniteModel
from aria.quant.fossil.naturalproofs.decl_api import get_vocabulary, is_var_decl
        ...
        if arity == 0:
            ...
            for input_arg in model[fct_name].keys():
                ...
                if isinstance(output_value, set):
                    output_value_converted = recover_value(
                        output_value, uct_signature[-1]
                    )
                else:
...
def generateConstraints(
    model, lemma_args, terms, is_true_constraint, annctx, instantiations=None
):
    const = [arg for arg in lemma_args if not is_var_decl(arg, annctx)]
    const_values = [
        model.smtmodel.eval(cs, model_completion=True).as_long()
        + (model.offset if is_expr_fg_sort(cs, annctx) else 0)
        for cs in const
    ]
    const_values = [
        "(- " + str(cv * -1) + ")" if cv < 0 else str(cv) for cv in const_values
    ]
    const_values = " ".join(const_values)
    constraints = ""
    lemma_arity = len(lemma_args) - len(const)
    eval_terms = {
        model.smtmodel.eval(term, model_completion=True).as_long() + model.offset
        for term in terms
    }
...
def getSygusOutput(
    ...
    if goal_fo_solver is None:
        raise Exception(
            "Something is wrong. A fixed solver object for the goal is needed. Consult an expert."
        )
    ...
    goal_extraction_terms = goal_npsolution.extraction_terms
    false_finitemodel = FiniteModel(
        goal_npsolution.model, goal_extraction_terms, annctx=annctx
    )
    ...
    false_model_relative_offset = (
        max(false_model_fg_universe) + abs(non_negative_offset) + 1
    )
    ...
    for cex_model in cex_models:
        ...
        if non_negative_offset >= 0:
            non_negative_offset = 0
        cex_offset_model.add_fg_element_offset(
            abs(non_negative_offset) + accumulated_offset
        )
        # Compute new accumulated offset
        accumulated_offset = (
            max(cex_model_universe) + abs(non_negative_offset) + accumulated_offset + 1
        )
    ...
    if true_cex_models:
        ...
        for true_cex_model in true_cex_models:
            ...
            if non_negative_offset >= 0:
                non_negative_offset = 0
            true_cex_offset_model.add_fg_element_offset(
                abs(non_negative_offset) + accumulated_offset
            )
            # Compute new accumulated offset
            accumulated_offset = (
                max(true_cex_model_universe)
                + abs(non_negative_offset)
                + accumulated_offset
                + 1
            )
            true_cex_model_term_universe = {
                IntVal(elem) for elem in true_cex_model_universe
            }
            instantiation_terms = {
                tuple(
                    elem.as_long() + true_cex_offset_model.offset
                    for elem in instantiation
                )
                for instantiation in true_cex_model[1]
            }
            true_cex_models_with_offset = true_cex_models_with_offset + [
                (
                    true_cex_offset_model,
                    true_cex_model_term_universe,
                    instantiation_terms,
                )
            ]
    ...
    all_models = (
        [cex_model.finitemodel for cex_model in cex_models]
        + [true_cex_model[0].finitemodel for true_cex_model in true_cex_models]
        + [false_finitemodel.finitemodel]
    )
    ...
    with open(out_file, "w") as out:
        ...
        if cex_models:
            cex_pfp_constraints = generateAllCexConstraints(
                cex_models, lemma_args, annctx
            )
            out.write(cex_pfp_constraints)
            out.write("\n")
        out.write("\n")
        out.write(";; constraints from false model\n")
        false_constraints = generateConstraints(
            false_finitemodel, lemma_args, goal_instantiation_terms, False, annctx
        )
        ...
        if true_cex_models:
            true_constraints = ""
            for true_cex_model in true_cex_models:
                curr_true_constraint = generateConstraints(
                    true_cex_model[0],
                    lemma_args,
                    true_cex_model[1],
                    True,
                    annctx,
                    instantiations=true_cex_model[2],
                )
    ...
    if options.streaming_synthesis_swtich:
        ...
        k_lemmas_file = "{}/{}_KLemmas.stream".format(
            options.log_file_path, problem_instance_name
        )
        if options.synthesis_solver != options.cvc4sy:
            raise RuntimeError("Streaming only supported wtih CVC4Sy.")
        ps = ProcessStreamer(
            cmdlist=["cvc4", "--lang=sygus2", "--sygus-stream", out_file],
        ...
        if options.synthesis_solver == options.minisy:
            proc = subprocess.Popen(
                "minisy {} --smtsolver=z3".format(out_file),
