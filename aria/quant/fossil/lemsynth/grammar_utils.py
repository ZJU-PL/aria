...
def representative_lemmas(lemma_args, lemma_grammar_terms, annctx, future=False):
    ...
    for recdef in bool_recdefs:
        if future:
            ...
            example_lemmas = example_lemmas.union(
                {
                    z3.Implies(recdef(*binding), example_lemma_body)
            ...
            example_lemmas.add(
                z3.Implies(recdef(*[lemma_args[: recdef.arity()]]), example_lemma_body)
            )
...
def lemma_instantiation_terms(lemma_args, lemma_grammar_terms, annctx):
    example_lemmas = representative_lemmas(
        lemma_args, lemma_grammar_terms, annctx=annctx
    )
...
def goal_extraction_terms(
    goal_instantiation_terms, lemma_args, lemma_grammar_terms, annctx
):
    example_lemmas = representative_lemmas(
        lemma_args, lemma_grammar_terms, annctx=annctx
    )
    ...
    bound_example_lemmas = {
        (lemma_params, example_lemma) for example_lemma in example_lemmas
    }
    ...
    example_extraction_terms = get_foreground_terms(
        example_instantiations, annctx=annctx
    )
