...
AddRecDefinition(
    red_height,
    x,
    If(x == nil, 1, If(red(x), 1 + red_height(nxt(x)), red_height(nxt(x)))),
)
AddRecDefinition(
    black_height,
    x,
    If(x == nil, 0, If(black(x), 1 + black_height(nxt(x)), black_height(nxt(x)))),
)
...
lemma_body = Implies(
    rlst(x),
    And(
        Implies(black(x), red_height(x) == black_height(x)),
