...
def set_timeout(func: Callable, timeout: int, *args, **kwargs) -> sp.Basic:
    ...
    def timeout_handler(signum, frame):
        raise TimeoutError(
            f"The operation {func.__name__} timed out after " f"{timeout} seconds."
        )
...
def to_smt(constraint: sp.Basic) -> str:
    ...
    if isinstance(constraint, Exists):
        var_str = " ".join([f"({to_smt(var)} Real)" for var in constraint.variables])
    ...
    if constraint.is_Relational:
        assert (
            len(constraint.args) == 2
        ), f"Expected 2 arguments, got {len(constraint.args)}"
    ...
    if constraint.is_Function and constraint.is_Boolean:
        ...
        if f == "not":
            assert (
                len(constraint.args) == 1
            ), f"Expected 1 argument, got {len(constraint.args)}"
            child = constraint.args[0]
            if isinstance(child, sp.Implies):
                assert (
                    len(child.args) == 2
                ), f"Expected 2 arguments, got {len(child.args)}"
