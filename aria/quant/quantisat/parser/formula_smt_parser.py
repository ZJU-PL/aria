...
def from_pysmt(expr) -> Quantifier:
    ...
    if expr.is_forall():
        ...
        return ForAll(
            [sp.Symbol(var.symbol_name()) for var in expr.quantifier_vars()],
            from_pysmt(expr.arg(0)),
        )
    elif expr.is_exists():
        assert len(expr.args()) == 1, f"Expected 1 argument, got {len(expr.args())}"
        return Exists(
        ...
        assert len(expr.args()) == 2, f"Expected 2 arguments, got {len(expr.args())}"
        return sp.Implies(
            from_pysmt(expr.arg(0)), from_pysmt(expr.arg(1)), evaluate=False
        )
    elif expr.is_and():
        assert (
            len(expr.args()) >= 2
        ), f"Expected more or equal to 2 arguments, got {len(expr.args())}"
