...
def signal_handler(sig, frame):
    ...
    logging.info(
        f"Received signal {signal_name} ({sig}), terminating child processes..."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--timeout", dest="timeout", default=8, type=int, help="timeout"
    )
    parser.add_argument(
        "--workers", dest="workers", default=4, type=int, help="workers"
    )
    parser.add_argument(
        "--verbose", dest="verbosity", default=0, type=int, help="verbosity"
    )
    parser.add_argument(
        "--logic", dest="logic", default="ALL", type=str, help="logic to use"
    )
    parser.add_argument(
        "--mode",
        dest="mode",
