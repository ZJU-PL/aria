...
class RBACModel:
    def __init__(self):
        self.users, self.roles, self.permissions, self.resources = (
            set(),
            set(),
            set(),
            set(),
    ...
    def check_user_permission(self, user, perm):
        ...
        return any(
            perm in self.get_all_permissions_for_role(r)
    ...
    def initialize_z3_variables(self):
        for user in self.users:
            ...
            self.z3_user_role[user] = {
                role: Bool(f"user_{user}_has_role_{role}") for role in self.roles
            }
        for role in self.roles:
            self.z3_roles[role] = Bool(f"role_{role}")
            self.z3_role_perm[role] = {
                perm: Bool(f"role_{role}_has_permission_{perm}")
                for perm in self.permissions
            }
            self.z3_role_hierarchy[role] = {
                jr: Bool(f"role_{role}_inherits_{jr}") for jr in self.roles
            }
        for perm in self.permissions:
            self.z3_permissions[perm] = Bool(f"permission_{perm}")
            self.z3_perm_resource[perm] = {
                res: Bool(f"permission_{perm}_on_resource_{res}")
        ...
        for user in self.users:
            for role in self.roles:
                self.solver.add(
                    ...
                    else Not(self.z3_user_role[user][role])
                )
        for role in self.roles:
            for perm in self.permissions:
                self.solver.add(
                    self.z3_role_perm[role][perm]
                    if perm in self.role_permission_assignments[role]
                    else Not(self.z3_role_perm[role][perm])
                )
        for perm in self.permissions:
            for res in self.resources:
                self.solver.add(
                    self.z3_perm_resource[perm][res]
                    if res in self.permission_resource_assignments[perm]
                    else Not(self.z3_perm_resource[perm][res])
                )
        for sr in self.roles:
            for jr in self.roles:
                self.solver.add(
                    self.z3_role_hierarchy[sr][jr]
                    if jr in self.role_hierarchy[sr]
                    else Not(self.z3_role_hierarchy[sr][jr])
                )
    ...
    def _extract_counterexample(self, model):
        ...
        for role in self.roles:
            ce["role_permissions"][role] = [
                perm
                for perm in self.permissions
                if is_true(model.evaluate(self.z3_role_perm[role][perm]))
            ]
        for perm in self.permissions:
            ce["permission_resources"][perm] = [
                res
                for res in self.resources
                if is_true(model.evaluate(self.z3_perm_resource[perm][res]))
            ]
        for sr in self.roles:
            ce["role_hierarchy"][sr] = [
                jr
                for jr in self.roles
                if is_true(model.evaluate(self.z3_role_hierarchy[sr][jr]))
            ]
        return ce
    ...
    def verify_separation_of_duty(self, r1, r2):
        ...
        prop = And(
            [
                Not(And(self.z3_user_role[u][r1], self.z3_user_role[u][r2]))
                for u in self.users
            ]
        )
    ...
    def verify_least_privilege(self, role, req_perms):
        ...
        not_others = And(
            [
                Not(self.z3_role_perm[role][p])
                for p in self.permissions
                if p not in req_perms
            ]
        )
    ...
    def verify_role_containment(self, r1, r2):
        ...
        prop = And(
            [
                Implies(self.z3_role_perm[r2][p], self.z3_role_perm[r1][p])
                for p in self.permissions
            ]
        )
        return self.verify_property(prop)
                ...
                Not(
                    And(
                        Or(
                            [
                                And(
                                    self.z3_user_role[u][role1],
                                    self.z3_role_perm[role1][p1],
                                    self.z3_perm_resource[p1][r1],
                                )
                                for role1 in self.roles
                                for p1 in self.permissions
                            ]
                        ),
                        Or(
                            [
                                And(
                                    self.z3_user_role[u][role2],
                                    self.z3_role_perm[role2][p2],
                                    self.z3_perm_resource[p2][r2],
                                )
                                for role2 in self.roles
                                for p2 in self.permissions
                            ]
                        ),
                    )
                )
