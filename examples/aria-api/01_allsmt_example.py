...
def bool_example():
    ...
    print(f"Boolean: {s.get_model_count()} models")
    s.print_models(False)
...
def int_example():
    ...
    print(f"Int: {s.get_model_count()} models")
    s.print_models(False)
...
def mixed_example():
    ...
    print(f"Mixed: {s.get_model_count()} models")
    s.print_models(False)
...
def compare_solvers():
    ...
    z3s = create_allsmt_solver("z3")
    z3s.solve(f, [a, b], model_limit=5)
    print(f"Z3: {z3s.get_model_count()} models")
    try:
        ps = create_allsmt_solver("pysmt")
        ps.solve(f, [a, b], model_limit=5)
        print(f"PySMT: {ps.get_model_count()} models")
    except:
        print("PySMT not available")
...
def real_example():
    ...
    print(f"Real: {s.get_model_count()} sample models")
    s.print_models(False)
